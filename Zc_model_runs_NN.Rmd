---
title: 'Cuvier''s beaked whale habitat models: Neural Net'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
```{r Ac. only Model 1, message = FALSE, eval = TRUE}
model1.indices <- c(1:5,7,10)
...
MSE.nn_AcOnly$v01_train <- sum((AcOnly_train_scaled$yAcOnly_TF - 
                  pr.nn_AcOnly.v01_train)^2)/nrow(AcOnly_train_scaled[model1.indices])
...
MSE.nn_AcOnly$v01_test <- sum((AcOnly_test_scaled$yAcOnly_TF - 
                   pr.nn_AcOnly.v01_test)^2)/nrow(AcOnly_test_scaled[model1.indices])
...
```{r other models 1, eval = TRUE, include = FALSE} 
## VISUAL ONLY
# put together the formula
...
MSE.nn_VisOnly$v01_train <- sum((VisOnly_train_scaled$yVisOnly_TF - 
...
MSE.nn_VisOnly$v01_test <- sum((VisOnly_test_scaled$yVisOnly_TF - 
...
tempNN <- nnet(f.Joint_NN1, data = Joint_train_scaled, 
                       size = 5, 
                       na.action = na.omit,
                       rang = 0.7,  
                       decay = 0.0001, 
...
MSE.nn_Joint$v01_train <- sum((Joint_train_scaled$y_TF - 
...
MSE.nn_Joint$v01_test <- sum((Joint_test_scaled$y_TF - 
...
```{r Ac. Only Model 2, message = FALSE, eval = TRUE}
# model2.indices <- c(1:5,7,11)
# nMax2 <- length(model2.indices)

# f.AcOnly_NN2 <- as.formula(paste("yAcOnly_TF ~", paste(n[model2.indices], collapse = " + ")))
...
MSE.nn_AcOnly$v02_train <- sum((AcOnly_train_scaled$yAcOnly_TF - 
                   pr.nn_AcOnly.v02_train)^2)/nrow(AcOnly_train_scaled[model1.indices])
MSE.nn_AcOnly$v02_train

# predict on test data and estimate MSE
pr.nn_AcOnly.v02_test <- predict(nn_AcOnly$v02,AcOnly_test_scaled[,model1.indices])
MSE.nn_AcOnly$v02_test <- sum((AcOnly_test_scaled$yAcOnly_TF - 
                   pr.nn_AcOnly.v02_test)^2)/nrow(AcOnly_test_scaled[model1.indices])
...
## VISUAL ONLY
# f.VisOnly_NN2 <- as.formula(paste("yVisOnly_TF ~", paste(n[model2.indices], collapse = " + ")))
...
MSE.nn_VisOnly$v02_train <- sum((VisOnly_train_scaled$yVisOnly_TF - 
...
MSE.nn_VisOnly$v02_test <- sum((VisOnly_test_scaled$yVisOnly_TF - 
                   pr.nn_VisOnly.v02_test)^2)/nrow(VisOnly_test_scaled[model1.indices])
MSE.nn_VisOnly$v02_test

## JOINT
# f.Joint_NN2 <- as.formula(paste("y_TF ~", paste(n[model2.indices], collapse = " + ")))
...
MSE.nn_Joint$v02_train <- sum((Joint_train_scaled$y_TF - 
...
MSE.nn_Joint$v02_test <- sum((Joint_test_scaled$y_TF - 
...
```{r Ac Only Model 3, message = FALSE, eval = TRUE}
# model3.indices <- c(1:5,7,12)
# nMax3 <- length(model3.indices)

# f.AcOnly_NN3 <- as.formula(paste("yAcOnly_TF ~", paste(n[model3.indices], collapse = " + ")))
...
MSE.nn_AcOnly$v03_train <- sum((AcOnly_train_scaled$yAcOnly_TF - 
                   pr.nn_AcOnly.v03_train)^2)/nrow(AcOnly_train_scaled[model1.indices])
MSE.nn_AcOnly$v03_train

# predict on test data and estimate MSE
pr.nn_AcOnly.v03_test <- predict(nn_AcOnly$v03,AcOnly_test_scaled[,model1.indices])
MSE.nn_AcOnly$v03_test <- sum((AcOnly_test_scaled$yAcOnly_TF - 
                   pr.nn_AcOnly.v03_test)^2)/nrow(AcOnly_test_scaled[model1.indices])
...
## Vis Only
#f.VisOnly_NN3 <- as.formula(paste("yVisOnly_TF ~", paste(n[model3.indices], collapse = " + ")))
...
MSE.nn_VisOnly$v03_train <- sum((VisOnly_train_scaled$yVisOnly_TF - 
...
MSE.nn_VisOnly$v03_test <- sum((VisOnly_test_scaled$yVisOnly_TF - 
                   pr.nn_VisOnly.v03_test)^2)/nrow(VisOnly_test_scaled[model1.indices])
MSE.nn_VisOnly$v03_test


## JOINT
#f.Joint_NN3 <- as.formula(paste("y_TF ~", paste(n[model3.indices], collapse = " + ")))
...
MSE.nn_Joint$v03_train <- sum((Joint_train_scaled$y_TF - 
...
MSE.nn_Joint$v03_test <- sum((Joint_test_scaled$y_TF - 
...
MSE.nn_AcOnly
```
Model 3 performs best on the training data, and model 2 performs best on the test data.
...
Model 1 performs best on both the train and test data.
...
Model 2 performs best on the training data and model 3 does best on the test data.
...
jan2009_AcOnly_prediction <- raster::predict(jan2009_rasters,nn_AcOnly$v02,
                                             na.action = na.pass)
            
july2009_AcOnly_prediction <- raster::predict(july2009_rasters,nn_AcOnly$v02,na.action = na.pass)    
```
<br>
...
jan2009_VisOnly_prediction <- raster::predict(jan2009_rasters,nn_VisOnly$v02,na.action = na.pass)
            
july2009_VisOnly_prediction <- raster::predict(july2009_rasters,nn_VisOnly$v02,
            na.action = na.pass)    
```
...
jan2009_prediction <- raster::predict(jan2009_rasters,nn_Joint$v02,na.action = na.pass)
            
july2009_prediction <- raster::predict(july2009_rasters,nn_Joint$v02,na.action = na.pass)    
```
<br>
...
```{r evaluate climatology-based models, eval = TRUE, message = FALSE, warning = FALSE}
...
for (iM in 1:length(monthStr)){
  climatePrediction[[iM]] <- raster::predict(raster_set[[iM]],nn_Joint$v02,
           na.action = na.pass)
