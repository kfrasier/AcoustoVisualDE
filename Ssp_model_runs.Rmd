...
yVisOnly <- ceiling(Train_VisOnly.set$Density)
...
Use distance to positive eddies:

```{r AC model 1, eval = FALSE}
glmm_full_AcOnly_TF<-NULL
glmm_full_AcOnly_TF$v01 <- glmmTMB(yAcOnly_TF~ SST + HYCOM_SALIN_0
                               + log10_HYCOM_MLD+(1|fac1),
                               data = transformedCovars_AcOnly.train, 
                               family = binomial, ziformula=~0)
glmm_full_AcOnly_TF$v01_1 <- glmmTMB(yAcOnly_TF~ SST + HYCOM_SALIN_0
                               + log10_HYCOM_MLD+(1|fac2),
                               data = transformedCovars_AcOnly.train, 
                               family = binomial, ziformula=~0)
glmm_full_AcOnly_TF$v01_2 <- glmmTMB(yAcOnly_TF~ SST + SSH + log10_HYCOM_MAG_0+ HYCOM_SALIN_0
                               + Pos_EddyDist + log10_HYCOM_MLD + (1|fac2),
                               data = transformedCovars_AcOnly.train, 
                               family = binomial, ziformula=~0)
glmm_full_AcOnly_TF$v01_3 <- glmmTMB(yAcOnly_TF~ SST + SSH + log10_HYCOM_MAG_0+ HYCOM_SALIN_0
                               + Pos_EddyDist + log10_HYCOM_MLD + (1|fac1),
                               data = transformedCovars_AcOnly.train, 
                               family = binomial, ziformula=~0)
glmm_full_AcOnly_TF$v01_4 <- glmmTMB(yAcOnly_TF~ SST + SSH + HYCOM_SALIN_0
                               + Pos_EddyDist + (1|fac2),
                               data = transformedCovars_AcOnly.train, 
                               family = binomial, ziformula=~0)
AICtab(glmm_full_AcOnly_TF$v01,glmm_full_AcOnly_TF$v01_1,glmm_full_AcOnly_TF$v01_2,
       glmm_full_AcOnly_TF$v01_3, glmm_full_AcOnly_TF$v01_4)
```


```{r other models 1, eval = FALSE, include = TRUE} 
gam_full_VisOnly_TF <-NULL
gam_full_VisOnly_TF$v01_1 <- gam(yVisOnly~ s(SST, bs='ts', k=kVal) 
                        ...
                        data = transformedCovars_VisOnly.train, 
                        family = ziP, na.action = na.omit)

glmm_full_VisOnly_TF$v01 <- glmmTMB(yVisOnly_TF~ SST + SSH + log10_HYCOM_MAG_0+ HYCOM_SALIN_0
                               + Pos_EddyDist + log10_HYCOM_MLD,
                               data = transformedCovars_VisOnly.train, 
                               family = binomial, ziformula=~1)


glmm_full_TF<-NULL
glmm_full_TF$v01 <- glmmTMB(y_TF~ SST+ SSH+log10_CHL+log10_HYCOM_MAG_0                    
                        + HYCOM_SALIN_0 + Pos_EddyDist +log10_HYCOM_MLD +(1|fac2),
                        data = transformedCovars.train, ziformula=~fac1,
                        family=binomial, dispformula=~0,
                        weights = mergedTrain.set$Weight/mean(mergedTrain.set$Weight))
glmm_full_TF$v01_1 <- glmmTMB(y_TF~log10_HYCOM_MLD +(1|fac1),
                        data = transformedCovars.train, ziformula=~fac1,
                        family=binomial,
                        weights = mergedTrain.set$Weight/mean(mergedTrain.set$Weight))
glmm_full_TF$v01_2 <- glmmTMB(y_TF~ SST+ SSH+log10_CHL+log10_HYCOM_MAG_0                    
                        + HYCOM_SALIN_0 + Pos_EddyDist +log10_HYCOM_MLD +(1|fac2),
                        data = transformedCovars.train, ziformula=~fac1,
                        family=binomial,
                        weights = mergedTrain.set$Weight/mean(mergedTrain.set$Weight))
glmm_full_TF$v01_3 <- glmmTMB(y_TF~ SST+ SSH+log10_CHL+log10_HYCOM_MAG_0                    
                        + HYCOM_SALIN_0 + Pos_EddyDist +log10_HYCOM_MLD +(1|fac1),
                        data = transformedCovars.train, ziformula=~fac3,
                        family=binomial,
                        weights = mergedTrain.set$Weight/mean(mergedTrain.set$Weight))
glmm_full_TF$v01_4 <- glmmTMB(y_TF~ SST+ SSH+log10_CHL+log10_HYCOM_MAG_0                    
                        + HYCOM_SALIN_0 + Pos_EddyDist +log10_HYCOM_MLD + (1|fac2),
                        data = transformedCovars.train, ziformula=~fac3,
                        family=binomial,dispformula=~0,
                        weights = mergedTrain.set$Weight/mean(mergedTrain.set$Weight))
AICtab(glmm_full_TF$v01,glmm_full_TF$v01_1,glmm_full_TF$v01_2,glmm_full_TF$v01_3
       ,glmm_full_TF$v01_4)
...
```{r AC model 02, eval = FALSE}
gamm_full_AcOnly_TF$v02 <- gam(yAcOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(EddyDist, bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
...
```{r Other models 02, eval = FALSE, include = TRUE}
gamm_full_VisOnly_TF$v02 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(EddyDist, bs='ts', k=kVal)
...
gamm_full_TF$v02 <- gamm(y_TF~ s(SST, bs='ts', k=kVal) 
                        ...
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal)
                        + s(log10_HYCOM_MLD, bs='ts', k=kVal),
                        data = transformedCovars.train,random = list(fac3=~1),
                        weights = mergedTrain.set$Weight/mean(mergedTrain.set$Weight),
                        na.action = na.omit,family = quasibinomial(),
                        niterPQL = 50)
```
...
save(AcOnly_gamm_pruned_TF_best,file = paste(outDir,SP,'_best_AcOnly_binomial_model.Rdata',sep=''))
...
VisOnly_gamm_pruned_TF_best <- gamm(yVisOnly_TF~ s(SSH,EddyDist, bs='ts', k=kVal)
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal),
                               data = transformedCovars_VisOnly.train,
                               na.action = na.omit,family = quasibinomial(),
                               correlation = corAR1(form=~1|fac2))
...
for (iM in 1:length(monthStr)){
  ...
  log10_HYCOM_MLD_climate <- log10(resample(raster(paste0(HYCOM_MLD_ClimateDir,'/',
                                        'proj_mld_month',
                                        monthNum[iM],'_mean.img')),predTemplate))
  raster_set[[iM]] <- brick(SST_climate,SSH_climate,log10_CHL_climate,
                            eddy_climate,neg_eddy_climate,pos_eddy_climate,
                            log10_HYCOM_MAG_climate,HYCOM_SALIN_climate,log10_HYCOM_MLD_climate) 
  names(raster_set[[iM]])<-c('SST','SSH','log10_CHL',
                             'EddyDist','Neg_EddyDist','Pos_EddyDist',
                             'log10_HYCOM_MAG_0','HYCOM_SALIN_0','log10_HYCOM_MLD')
