...
Note: Because there are so few sightings, it seems the visual data are not predicting well.
...
Spatial acousic-only model predictions using oceanographic variables averaged by month between 2003 and 2015.

```{r evaluate acoustic only climatology models, echo = FALSE, message = FALSE, warning = FALSE}
load('E:/NASData/AcoustoVisualDE/AcoustoVisualDE/climatology_rasters_scaled.Rdata') 
monthNum <-c('01','02','03','04','05','06','07','08','09','10','11','12')
monthStr<-c('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec')

climatePrediction_AcOnly <- vector('list',length = 12)
mapClimatePrediction_AcOnly <- vector('list',length = 12)

for (iM in 1:length(monthStr)){
  climatePrediction_AcOnly[[iM]] <- raster::predict(raster_set[[iM]],
      nn_AcOnly[[best_Joint_ModelIndex]],na.action = na.pass)
  mapClimatePrediction_AcOnly[[iM]] <- mask(projectRaster(
    climatePrediction_AcOnly[[iM]], crs=map_proj),predict_template_proj)  
  # output raster to geotiff
  rasterImageFileName_AcOnly = paste0(savePath,'/climatology_predictions/',
      SP,'_',monthStr[iM],'NN_mean_encounter_probability_AcOnly.tif')
  writeRaster(mapClimatePrediction_AcOnly[[iM]],
      filename = rasterImageFileName_AcOnly,format="GTiff",overwrite = TRUE)
  
  #ouput raster to kml
  kmlImageFileName_AcOnly = paste0(savePath,'/climatology_predictions/',
      SP,'_',monthStr[iM],'NN_mean_encounter_probability_AcOnly.kml')
  KML(mapClimatePrediction_AcOnly[[iM]],
      file = kmlImageFileName_AcOnly, col=matlab.like2(32),
      overwrite = TRUE)
}
  
```

```{r plot acoustic only climatologies, message = FALSE, warning = FALSE, echo = FALSE}
pal <- colorNumeric(palette = matlab.like2(5),
                    domain=c(0,1), 
                    na.color = 'transparent')
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  addProviderTiles(providers$Esri.OceanBasemap) %>%
  addRasterImage(mapClimatePrediction_AcOnly[[1]] , colors = pal,
                 opacity = 0.8, group = 'Jan.') %>%
  addRasterImage(mapClimatePrediction_AcOnly[[3]] , colors = pal,
                 opacity = 0.8, group = 'March') %>%
  addRasterImage(mapClimatePrediction_AcOnly[[5]] , colors = pal,
                 opacity = 0.8, group = 'May') %>%
  addRasterImage(mapClimatePrediction_AcOnly[[7]] , colors = pal,
                 opacity = 0.8, group = 'July') %>%
  addRasterImage(mapClimatePrediction_AcOnly[[9]] ,colors = pal,
                 opacity = 0.8, group = 'Sept.') %>%
  addRasterImage(mapClimatePrediction_AcOnly[[11]] ,colors = pal,
                 opacity = 0.8, group = 'Nov.') %>%
  addMarkers(data = HARPsites, lng = ~ long, lat = ~ lat, 
                 group = "HARP Locations") %>%
  addLegend(pal = pal, values = c(0,1),
