...
outDir <- file.path('E:/NASData/ModelData',SP,'newGAM_density//')
...
clevlandJointTransformed<-paste0(outDir,
...
```{r Joint model summary,echo = FALSE, warning = FALSE}
...
plot(residuals.gam(Joint_model)*Joint_model$prior.weights)
...
MAE$AcOnly_test_JointData <- mean(abs(as.vector(transformedCovars.test$y)-
                                    pr$AcOnly_JointData$fit),na.rm = TRUE)
MAE$VisOnly_test_JointData <- mean(abs(as.vector(transformedCovars.test$y)-
                                    pr$VisOnly_JointData$fit),na.rm = TRUE)
MAE$Joint_test <- mean(abs(as.vector(transformedCovars.test$y)-
                         pr$Joint_JointData$fit),na.rm = TRUE)

wMAE <- NULL

wMAE$AcOnly_test_JointData <- mean(as.vector(joint_test_weightsG0)*abs(as.vector(transformedCovars.test$y)-
                                    pr$AcOnly_JointData$fit),na.rm = TRUE)
wMAE$VisOnly_test_JointData <- mean(as.vector(joint_test_weightsG0)*abs(as.vector(transformedCovars.test$y)-
                                    pr$VisOnly_JointData$fit),na.rm = TRUE)
wMAE$Joint_test <- mean(as.vector(joint_test_weightsG0)*abs(as.vector(transformedCovars.test$y)-
                         pr$Joint_JointData$fit),na.rm = TRUE)
...
RMSE$Joint_test <- sqrt(mean(as.vector((transformedCovars.test$y)-
                                    pr$Joint_JointData$fit)^2,na.rm = TRUE))

wRMSE <- NULL

wRMSE$AcOnly_test_JointData <- sqrt(mean(as.vector(joint_test_weightsG0)*
                                    ((as.vector(transformedCovars.test$y)-
                                    pr$AcOnly_JointData$fit)^2),na.rm = TRUE))
wRMSE$VisOnly_test_JointData <- sqrt(mean(as.vector(joint_test_weightsG0)*
                                    ((as.vector(transformedCovars.test$y)-
                                    pr$VisOnly_JointData$fit)^2),na.rm = TRUE))
wRMSE$Joint_test <- sqrt(mean(as.vector(joint_test_weightsG0)*
                                    ((as.vector(transformedCovars.test$y)-
                                    pr$Joint_JointData$fit)^2),na.rm = TRUE))

RMSETable <- rbind(RMSE$AcOnly_test_JointData,RMSE$VisOnly_test_JointData,RMSE$Joint_test)
wRMSETable <- rbind(wRMSE$AcOnly_test_JointData,wRMSE$VisOnly_test_JointData,wRMSE$Joint_test)
MAETable<- rbind(MAE$AcOnly_test_JointData,MAE$VisOnly_test_JointData,MAE$Joint_test)
wMAETable<- rbind(wMAE$AcOnly_test_JointData,wMAE$VisOnly_test_JointData,wMAE$Joint_test)

colnames(MAETable)<- 'MAE'#'Mean Absolute Error'
rownames(wRMSETable)<- c("Acoustic - Test Joint Data",
                       "Visual - Test Joint Data",
                       "Joint - Test Joint Data")
colnames(wMAETable)<- 'wMAE'
rownames(wMAETable)<- c("Acoustic - Test Joint Data",
                       "Visual - Test Joint Data",
                       "Joint - Test Joint Data")
colnames(RMSETable)<- 'RMSE'
rownames(RMSETable)<- c("Acoustic - Test Joint Data",
                       "Visual - Test Joint Data",
                       "Joint - Test Joint Data")
colnames(wRMSETable)<- 'wRMSE'
rownames(wRMSETable)<- c("Acoustic - Test Joint Data",
                       "Visual - Test Joint Data",
                       "Joint - Test Joint Data")

print('Mean absolute error scores (lower is better)')
print(round(MAETable, digits = 3))
print('Weighted mean absolute error scores (lower is better)')
print(round(wMAETable, digits = 3))
print('RMSE scores (lower is better)')
print(round(RMSETable, digits = 3))
print('Weighted RMSE scores (lower is better)')
print(round(wRMSETable, digits = 3))
...
Ac_predictionSet$DC <- NA
Ac_predictionSet$DC[m4] <- allCovars$A_pred[compAcSet_DC_all]
Ac_predictionSet$predSd_DC <- NA
Ac_predictionSet$predSd_DC[m4] <- allCovars$A_predSd[compAcSet_DC_all]
Ac_predictionSet$DC_data_smooth <- NA
Ac_predictionSet$DC_data_smooth[m4] <- rollmean(allCovars$observations[compAcSet_DC_all],k = 7,fill=NA)
Ac_predictionSet$DC_data <- NA
Ac_predictionSet$DC_data[m4] <- allCovars$observations[compAcSet_DC_all]

MP_times <- allCovars$date[compAcSet_MP_all]
m5 <- match(DC_times,allTheDays)
Ac_predictionSet$MP<- NA
Ac_predictionSet$MP[m5] <- allCovars$A_pred[compAcSet_MP_all]
Ac_predictionSet$predSd_MP <- NA
Ac_predictionSet$predSd_MP[m5] <- allCovars$A_predSd[compAcSet_MP_all]
Ac_predictionSet$MP_data_smooth <- NA
Ac_predictionSet$MP_data_smooth[m5] <- rollmean(allCovars$observations[compAcSet_MP_all],k = 7,fill=NA)
Ac_predictionSet$MP_data <- NA
Ac_predictionSet$MP_data[m5] <- allCovars$observations[compAcSet_MP_all]

Ac_predictionSet$Legend1 <- "Observations"
Ac_predictionSet$StDev <- "Std. Error"
Ac_predictionSet$Legend <- "Predictions - Train"
allCovars$Legend <- "Predictions"
allCovars$DataGroup <- "Observations"
allCovars$DE <- "Density"
Ac_predictionSet$LegendTest <- "Predictions - Test"
```
...
p1_Ac <- ggplot() +
  # geom_ribbon(data = Ac_predictionSet,
  #           aes(x = allTheDays, ymax = predSd_MC + MC, ymin = pmax(MC - predSd_MC,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = MC_data, color = Legend1),alpha = .5) +
  geom_path(data = Ac_predictionSet,
            aes(x= allTheDays, y = MC, color = Legend),ymax = myYmax) +
  geom_path(data = Ac_predictionSet[Ac_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = MC, color = LegendTest,
            ymax = myYmax)) +
  labs(y = "", x = NULL)+
...
p2_Ac <- ggplot() +
  # geom_ribbon(data = Ac_predictionSet,
  #           aes(x = allTheDays, ymax = predSd_GC + GC, ymin = pmax(GC - predSd_GC,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = GC_data, color = Legend1),alpha = .5) +
  geom_path(data = Ac_predictionSet,
            aes(x= allTheDays, y = GC, color = Legend, ymax = myYmax)) +
  geom_path(data = Ac_predictionSet[Ac_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = GC, color = LegendTest,
            ymax = myYmax)) +
...
p3_Ac <- ggplot() +
  # geom_ribbon(data = Ac_predictionSet,
  #           aes(x = allTheDays, ymax = predSd_DT + DT, ymin = pmax(DT - predSd_DT,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = DT_data, color = Legend1),alpha = .5) + 
  geom_path(data = Ac_predictionSet,
            aes(x= allTheDays, y=DT, color = Legend, ymax = myYmax)) +
  geom_path(data = Ac_predictionSet[Ac_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = DT, color = LegendTest,
            ymax = myYmax)) +
  scale_color_manual(values= c("#A0A0A0","#009900","#0000FF"))+
...
p4_Ac <- ggplot() +
  # geom_ribbon(data = Ac_predictionSet,
  #           aes(x = allTheDays, ymax = predSd_DC + DC, ymin = pmax(DC - predSd_DC,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = DC_data, color = Legend1),alpha = .5) + 
  geom_path(data = Ac_predictionSet,
            aes(x= allTheDays, y=DC, color = Legend, ymax = myYmax)) +
  geom_path(data = Ac_predictionSet[Ac_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = DC, color = LegendTest,
            ymax = myYmax)) +
  scale_color_manual(values= c("#A0A0A0","#009900","#0000FF"))+
  labs(y = "", x = NULL)+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,))+
...
p5_Ac <- ggplot() +
  # geom_ribbon(data = Ac_predictionSet,
  #           aes(x = allTheDays, ymax = predSd_MP + MP, ymin = pmax(MP - predSd_MP,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = MP_data, color = Legend1),alpha = .5) + 
  geom_path(data = Ac_predictionSet,
            aes(x= allTheDays, y=MP, color = Legend, ymax = myYmax)) +
  geom_path(data = Ac_predictionSet[Ac_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = MP, color = LegendTest,
            ymax = myYmax)) +
  scale_color_manual(values= c("#A0A0A0","#009900","#0000FF"))+
  labs(y = "", x = NULL)+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,))+
...
png(paste(outDir,SP,'_AcOnly_Timeseries_Prediction_gam_Density.png',sep=''), width = 6.5, height = 4, units = "in", res=300)
...
DC_times <- allCovars$date[compAcSet_DC_all]
m4 <- match(DC_times,allTheDays)
Vis_predictionSet$DC<- NA
Vis_predictionSet$DC[m4] <- allCovars$V_pred[compAcSet_DC_all]
Vis_predictionSet$predSd_DC<- NA
Vis_predictionSet$predSd_DC[m4] <- allCovars$V_predSd[compAcSet_DC_all]
Vis_predictionSet$DC_data_smooth <- NA
Vis_predictionSet$DC_data_smooth[m4] <- rollmean(allCovars$observations[compAcSet_DC_all],k = 7,fill=NA)

MP_times <- allCovars$date[compAcSet_MP_all]
m5 <- match(MP_times,allTheDays)
Vis_predictionSet$MP<- NA
Vis_predictionSet$MP[m5] <- allCovars$V_pred[compAcSet_MP_all]
Vis_predictionSet$predSd_MP<- NA
Vis_predictionSet$predSd_MP[m5] <- allCovars$V_predSd[compAcSet_MP_all]
Vis_predictionSet$MP_data_smooth <- NA
Vis_predictionSet$MP_data_smooth[m5] <- rollmean(allCovars$observations[compAcSet_MP_all],k = 7,fill=NA)


Vis_predictionSet$Legend1 <- "Observations"
Vis_predictionSet$StDev <- "Std. Error"
Vis_predictionSet$Legend <- "Predictions - Train"
Vis_predictionSet$LegendTest <- "Predictions - Test"
...
p1_Vis <- ggplot() +
  # geom_ribbon(data = Vis_predictionSet,
  #           aes(x = allTheDays, ymax = predSd_MC + MC, ymin = pmax(MC - predSd_MC,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = MC_data, color = Legend1),alpha = .5) +
  geom_path(data = Vis_predictionSet,
            aes(x= allTheDays, y = MC, color = Legend, 
            ymax = myYmax)) +
  geom_path(data = Vis_predictionSet[Vis_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = MC, color = LegendTest,
            ymax = myYmax)) +
  labs(y = "", x = "")+
    scale_color_manual(values= c("#A0A0A0","#009900","#0000FF"))+theme_bw()+
...
p2_Vis <- ggplot() +
  ...
  #           aes(x = allTheDays, ymax = predSd_GC + GC, ymin = pmax(GC - predSd_GC,0)), fill = "lightgray") +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = GC_data, color = Legend1, 
            ymax = myYmax),alpha = .5) +
  geom_path(data = Vis_predictionSet,
            aes(x= allTheDays, y=GC, color = Legend)) +
  geom_path(data = Vis_predictionSet[Vis_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = GC, color = LegendTest,
            ymax = myYmax)) +
  scale_color_manual(values= c("#A0A0A0","#009900","#0000FF"))+
...
p3_Vis <- ggplot() +
  ...
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = DT_data, color = Legend1),alpha = .5) +  
  geom_path(data = Vis_predictionSet,
            aes(x= allTheDays, y=DT, color = Legend, 
            ymax = myYmax)) +
  geom_path(data = Vis_predictionSet[Vis_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = DT, color = LegendTest,
            ymax = myYmax)) +
...
p4_Vis <- ggplot() +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = DC_data, color = Legend1),alpha = .5) +  
  geom_path(data = Vis_predictionSet,
            aes(x= allTheDays, y=DC, color = Legend, 
            ymax = myYmax)) +
  geom_path(data = Vis_predictionSet[Vis_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = DC, color = LegendTest,
            ymax = myYmax)) +
...
p5_Vis <- ggplot() +
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = MP_data, color = Legend1),alpha = .5) +  
  geom_path(data = Vis_predictionSet,
            aes(x= allTheDays, y=MP, color = Legend, 
            ymax = myYmax)) +
  geom_path(data = Vis_predictionSet[Vis_predictionSet$allTheDays>="2013-1-1",],
             aes(x = allTheDays, y = MP, color = LegendTest,
            ymax = myYmax)) +
...
png(paste(outDir,SP,'_VisOnly_Timeseries_Prediction_gam_Density.png',sep=''), width = 6.5, height = 4, units = "in", res=300)
...
predJoint <- (predict.gam(Joint_model,
                             allCovars,
                             type = 'response',na.action = na.pass,se.fit=TRUE))
allCovars$J_pred <- predJoint$fit
allCovars$J_predSd <- predJoint$se.fit

Joint_predictionSet <- NA
Joint_predictionSet<- data.frame(allTheDays)
MC_times <- allCovars$date[compAcSet_MC_all]
m1 <- match(MC_times,allTheDays)
Joint_predictionSet$MC<- NA
Joint_predictionSet$MC[m1] <- allCovars$J_pred[compAcSet_MC_all]
Joint_predictionSet$predSd_MC<- NA
Joint_predictionSet$predSd_MC[m1] <- allCovars$J_predSd[compAcSet_MC_all]
Joint_predictionSet$MC_data_smooth <- NA
Joint_predictionSet$MC_data_smooth[m1] <- rollmean(allCovars$observations[compAcSet_MC_all],k = 7,fill=NA)

GC_times <- allCovars$date[compAcSet_GC_all]
m2 <- match(GC_times,allTheDays)
Joint_predictionSet$GC<- NA
Joint_predictionSet$GC[m2] <- allCovars$J_pred[compAcSet_GC_all]
Joint_predictionSet$predSd_GC<- NA
Joint_predictionSet$predSd_GC[m2] <- allCovars$J_predSd[compAcSet_GC_all]
Joint_predictionSet$GC_data_smooth <- NA
Joint_predictionSet$GC_data_smooth[m2] <- rollmean(allCovars$observations[compAcSet_GC_all],k = 7,fill=NA)

DT_times <- allCovars$date[compAcSet_DT_all]
m3 <- match(DT_times,allTheDays)
Joint_predictionSet$DT<- NA
Joint_predictionSet$DT[m3] <- allCovars$J_pred[compAcSet_DT_all]
Joint_predictionSet$predSd_DT<- NA
Joint_predictionSet$predSd_DT[m3] <- allCovars$J_predSd[compAcSet_DT_all]
Joint_predictionSet$DT_data_smooth <- NA
Joint_predictionSet$DT_data_smooth[m3] <- rollmean(allCovars$observations[compAcSet_DT_all],k = 7,fill=NA)

DC_times <- allCovars$date[compAcSet_DC_all]
m4 <- match(DC_times,allTheDays)
Joint_predictionSet$DC<- NA
Joint_predictionSet$DC[m4] <- allCovars$J_pred[compAcSet_DC_all]
Joint_predictionSet$predSd_DC<- NA
Joint_predictionSet$predSd_DC[m4] <- allCovars$J_predSd[compAcSet_DC_all]
Joint_predictionSet$DC_data_smooth <- NA
Joint_predictionSet$DC_data_smooth[m4] <- rollmean(allCovars$observations[compAcSet_DC_all],k = 7,fill=NA)

MP_times <- allCovars$date[compAcSet_MP_all]
m5 <- match(MP_times,allTheDays)
Joint_predictionSet$MP<- NA
Joint_predictionSet$MP[m5] <- allCovars$J_pred[compAcSet_MP_all]
Joint_predictionSet$predSd_MP<- NA
Joint_predictionSet$predSd_MP[m5] <- allCovars$J_predSd[compAcSet_MP_all]
Joint_predictionSet$MP_data_smooth <- NA
Joint_predictionSet$MP_data_smooth[m5] <- rollmean(allCovars$observations[compAcSet_MP_all],k = 7,fill=NA)
...
p1 <- ggplot() +
  ...
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = MC_data, color = Legend1), alpha = 1,width = .1,size = .25)+
  geom_line(data = Ac_predictionSet,
            aes(x=allTheDays, y=MC, color = Legend), size = .5) +
  geom_path(data = Joint_predictionSet,
            aes(x=allTheDays, y=MC),color = "red2", size = .5) +
  geom_path(data = Vis_predictionSet,
            aes(x=allTheDays, y=MC, color = LegendTest),size = .5) +
  labs(y = "", x = NULL)+coord_cartesian(ylim = c(0,25))+
  scale_color_manual(values= c("grey35","green3","deepskyblue"))+theme_bw()+
...
p2 <- ggplot() +
  geom_rect(data =Vis_predictionSet, aes(xmin=allTheDays[731], xmax=allTheDays[1095], ymin=0, ymax=25), fill="gray85")+
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = GC_data, color = Legend1), alpha = 1,width = .1,size = .25)+
  geom_line(data = Ac_predictionSet,
            aes(x=allTheDays, y=GC, color = Legend), size = .5) +
  geom_path(data = Joint_predictionSet,
            aes(x=allTheDays, y=GC),color = "red2", size = .5) +
  geom_path(data = Vis_predictionSet,
            aes(x=allTheDays, y=GC, color = LegendTest),size = .5) +
  labs(y = "", x = NULL)+coord_cartesian(ylim = c(0,25))+
  scale_color_manual(values= c("grey35","green3","deepskyblue"))+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,10,12), "pt"),
        axis.text.x = element_blank())+
...
p3 <- ggplot() +
  ...
  geom_line(data = Ac_predictionSet,
            aes(x=allTheDays, y=DT, color = Legend), size = .5) +
  geom_path(data = Joint_predictionSet,
            aes(x=allTheDays, y=DT),color = "red2", size = .5) +
  geom_path(data = Vis_predictionSet,
...
p4 <- ggplot() +
  geom_rect(data =Vis_predictionSet, aes(xmin=allTheDays[731], xmax=allTheDays[1095], ymin=0, ymax=25), fill="gray85")+
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = DC_data, color = Legend1), alpha = 1,width = .1,size = .25)+
  geom_line(data = Ac_predictionSet,
            aes(x=allTheDays, y=DC, color = Legend), size = .5) +
  geom_path(data = Joint_predictionSet,
            aes(x=allTheDays, y=DC),color = "red2", size = .5) +
  geom_path(data = Vis_predictionSet,
            aes(x=allTheDays, y=DC, color = LegendTest),size = .5) +
  labs(y = "", x = NULL)+coord_cartesian(ylim = c(0,25))+
  scale_color_manual(values= c("grey35","green3","deepskyblue"))+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,10,12), "pt"),
        axis.text.x = element_blank())+
...
p5 <- ggplot() +
  geom_rect(data =Vis_predictionSet, aes(xmin=allTheDays[731], xmax=allTheDays[1095], ymin=0, ymax=25), fill="gray85")+
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = MP_data, color = Legend1), alpha = 1,width = .1,size = .25)+
  geom_line(data = Ac_predictionSet,
            aes(x=allTheDays, y=MP, color = Legend), size = .5) +
  geom_path(data = Joint_predictionSet,
            aes(x=allTheDays, y=MP),color = "red2", size = .5) +
  geom_path(data = Vis_predictionSet,
            aes(x=allTheDays, y=MP, color = LegendTest),size = .5) +
  labs(y = "", x = NULL)+coord_cartesian(ylim = c(0,25))+
  scale_color_manual(values= c("grey35","green3","deepskyblue"))+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,10,12), "pt"),
        axis.text.x = element_blank())+
...
png(paste(outDir,SP,'_Joint_Timeseries_Prediction_gam_Density.png',sep=''), width = 6.5, height = 4, units = "in", res=300)
...
load('E:/NASData/AcoustoVisualDE/AcoustoVisualDE/2009_prediction_rasters_UNscaled.Rdata') 
```
...
# compute standard error raster. Have to convert to points and back to make this work.
july2009_rastersJoint_points = na.omit(rasterToPoints(july2009_rastersJoint))
july2009_joint_se_predict_points = predict.gam(Joint_model,data.frame(july2009_rastersJoint_points),
            type = 'response', na.action = na.pass,se.fit = TRUE)
myXY = cbind(july2009_rastersJoint_points[,1],july2009_rastersJoint_points[,2])
july2009_prediction_se = rasterize(myXY,july2009_prediction,july2009_joint_se_predict_points$se.fit,na.rm = TRUE)/10
```
...
july2009_prediction_se_crop = mask(july2009_prediction_se,crop_limits_proj)
...
JointMapSummer = tm_shape(july2009_prediction_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColor/2,title = expression("Animals/100"~km^{2}),legend.reverse = TRUE)+ 
...
maxColorSE <- ceiling(max(c(july2009_AcOnly_prediction_se_crop@data@max,
               july2009_VisOnly_prediction_se_crop@data@max,
               july2009_prediction_se_crop@data@max))*10)/10
# plot best summer map for publication
sightShape = SpatialPoints(sightingsTest[c(2,1)],proj4string =  crs(GU2009EffortLines))
harpShape = SpatialPoints(HARPsites[c(1,8,14),c(2,1)],proj4string =  
                            crs(GU2009EffortLines))
...
JointMapSummer = tm_shape(july2009_prediction_se_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(Density)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
