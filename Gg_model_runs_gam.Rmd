---
title: "GoMx Risso's dolphin habitat models: Presence/absence with GAMs"
author: 
...
The time series below show timeseries of estimated densities from passive acoustic data used for modeling (Densities were calculated following methods detailed in @RN806). 
...
MCTS <- paste0(savePath,'/',SP,'_Timeseries_Site_MC.png')
GCTS <- paste0(savePath,'/',SP,'_Timeseries_Site_GC.png')
DTTS <- paste0(savePath,'/',SP,'_Timeseries_Site_DT.png')
DCTS <- paste0(savePath,'/',SP,'_Timeseries_Site_DC.png')
MPTS <- paste0(savePath,'/',SP,'_Timeseries_Site_MP.png')
```
...
ACclevelandPlot<-paste0(savePath,'/AcousticOnly_',
                        SP,'_clevelandDots_noTransform.png')
VisclevelandPlot<-paste0(savePath,'/VisualOnly_',
                        SP,'_clevelandDots_noTransform.png')
```
...
dev.off()
covarPlot <-paste0(savePath,'/',SP,'_correlations_noTransform.png')
```
<br> 
...
visDetProbData<-paste0(savePath,'/',SP,'sightwTrunc_GU.Rdata')
load(visDetProbData)
visDetProb <- detFun[[bestModelIdx]]$fitted[1]
visDetProbFigure <- paste0(savePath,'/',SP,'sightwTrunc_GU.png')
```
...
\[P_{A}(detect|present) = (1-`r Ac_pDet`)^{288} \approx 0\]
...
png(paste(outDir,SP,'_caseWeights_gam.png',sep=''), width = 4, height = 4, units = 'in',res = 300)
...
```{r AcOnly model, eval = TRUE, echo = TRUE, warning = FALSE}
...
```{r VisOnly model, eval = TRUE, include = TRUE, echo = TRUE, warning = FALSE} 
...
```{r Joint model, eval = TRUE, include = TRUE, echo = TRUE, warning = FALSE} 
...
```{r AcOnly unpruned model summary, echo = FALSE, warning = FALSE}
...
png(paste(outDir,SP,'_Joint_smooths_pres_abs.png',sep=''), res = 300, units = "in", width = 5, height = ceil(nTerms/4)*1.5)
...
XEntropy$AcOnly_test_JointData <- weighted_logloss(round(pr$AcOnly_JointData$fit,
                                                         digits = 4),
                                 as.vector(transformedCovars.test$y_TF),
                                 as.vector(joint_test_weightsG0))
XEntropy$VisOnly_test_JointData <- weighted_logloss(round(pr$VisOnly_JointData$fit,
                                                          digits = 4),
                                 as.vector(transformedCovars.test$y_TF),
                                 as.vector(joint_test_weightsG0))
XEntropy$Joint_test <- weighted_logloss(round(pr$Joint_JointData$fit, digits = 4),
                                 as.vector(transformedCovars.test$y_TF),
                                 as.vector(joint_test_weightsG0))

XEntropyTable<- rbind(XEntropy$AcOnly_test_JointData,XEntropy$VisOnly_test_JointData,
                      XEntropy$Joint_test)

colnames(XEntropyTable)<- 'Cross-Entropy Score'
...
VisOnly_predictionSet$StDev <- "Std. Error"
pOccur$Legend <- "Observations"
```
...
allObservations[allObservations>0] <-1
predJoint <- (predict.gam(Joint_model,
                             allCovars,
                             type = 'response',na.action = na.pass,se.fit=TRUE))
allCovars$pred <- predJoint$fit
allCovars$predSd <- predJoint$se.fit
allCovars$observations <- allObservations
# predJoint_GC <- predict.gam(Joint_model,
#                              allCovars[compAcSet_GC_all,],
#                              type = 'response',na.action = na.pass,se.fit=TRUE)
# predJoint_DT <- predict.gam(Joint_model,
#                              allCovars[compAcSet_DT_all,],
#                              type = 'response',na.action = na.pass,se.fit=TRUE)
# predJoint_DC <- predict.gam(Joint_model,
#                               allCovars[compAcSet_DC_all,],
#                               type = 'response',na.action = na.pass,se.fit=TRUE)
# predJoint_MP <- predict.gam(Joint_model,
#                               allCovars[compAcSet_MP_all,],
#                               type = 'response',na.action = na.pass,se.fit=TRUE)

Joint_predictionSet <- data.frame(unique(c(Train_AcOnly.set$date,
                                    Test_AcOnly.set$date)))
# colnames(Joint_predictionSet) <-"date"
# Joint_predictionSet$MC <- NA
# Joint_predictionSet$predSd_MC <- NA
# MC_times <- Joint_predictionSet$date[compAcSet_MC_all]
# m1 <- na.omit(match(MC_times,Joint_predictionSet$date))
...
# GC_times <- Joint_predictionSet$date[compAcSet_GC_all]
# m2 <- na.omit(match(GC_times,Joint_predictionSet$date))
...
# DT_times <- Joint_predictionSet$date[compAcSet_DT_all]
# m3 <-  na.omit(match(DT_times,Joint_predictionSet$date))
...
# DC_times <- Joint_predictionSet$date[compAcSet_DC_all]
# m4 <-  na.omit(match(DC_times,Joint_predictionSet$date))
...
# MP_times <- Joint_predictionSet$date[compAcSet_MP_all]
# m5 <-  na.omit(match(MP_times,Joint_predictionSet$date))
...
# par(mfrow = c(5,1),oma=c(3,0,5,0)),colour="#000099",colour="#CC0000"
p1 <- ggplot() +
  geom_ribbon(data = allCovars[compAcSet_MC_all,],
            aes(x = date, ymax = predSd + pred, ymin = pmax(pred - predSd,0)), fill = "lightgray") +
  geom_area(data = allCovars[compAcSet_MC_all,], 
            aes(x = date, y = pmax(pred - predSd,0)), fill = "white") +
  geom_path(data = allCovars[compAcSet_MC_all,],
            aes(x= date, y=pred, color = Legend)) +
  geom_point(data = allCovars[compAcSet_MC_all,],
             aes(x = date, y = observations, color = DataGroup),alpha = 0.5) +
  labs(y = "Encounter probability", x = "")+
  scale_color_manual("",values= c("gray48","#009999"))+
  theme_bw()+theme(legend.position = c(0.9, 0.8),legend.title=element_blank(),
                   legend.margin=margin(c(1,5,5,5)))+
...
p2 <- ggplot() +
  geom_ribbon(data = allCovars[compAcSet_GC_all,],
            aes(x = date, ymax = predSd + pred, ymin = pmax(pred - predSd,0)), fill = "lightgray") +
  geom_area(data = allCovars[compAcSet_GC_all,], 
            aes(x = date, y = pmax(pred - predSd,0)), fill = "white") +
  geom_path(data = allCovars[compAcSet_GC_all,],
            aes(x= date, y=pred, color = Legend)) +
  geom_point(data = allCovars[compAcSet_GC_all,],
             aes(x = date, y = observations, color = DataGroup),alpha = 0.5) +
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "")+theme_bw()+
  theme(legend.position="none")+annotation_custom(gcLabel)
...
p3 <- ggplot() +
  geom_ribbon(data = allCovars[compAcSet_DT_all,],
            aes(x = date, ymax = predSd + pred, ymin = pmax(pred - predSd,0)), fill = "lightgray") +
  geom_area(data = allCovars[compAcSet_DT_all,], 
            aes(x = date, y = pmax(pred - predSd,0)), fill = "white") +
  geom_path(data = allCovars[compAcSet_DT_all,],
            aes(x= date, y=pred, color = Legend)) +
  geom_point(data = allCovars[compAcSet_DT_all,],
             aes(x = date, y = observations, color = DataGroup),alpha = 0.5) +
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "")+theme_bw()+
  theme(legend.position="none")+annotation_custom(dtLabel)
...
p4 <- ggplot() +
  geom_ribbon(data = allCovars[compAcSet_DC_all,],
            aes(x = date, ymax = predSd + pred, ymin = pmax(pred - predSd,0)), fill = "lightgray") +
  geom_area(data = allCovars[compAcSet_DC_all,], 
            aes(x = date, y = pmax(pred - predSd,0)), fill = "white") +
  geom_path(data = allCovars[compAcSet_DC_all,],
            aes(x= date, y=pred, color = Legend)) +
  geom_point(data = allCovars[compAcSet_DC_all,],
             aes(x = date, y = observations, color = DataGroup),alpha = 0.5) +
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "")+theme_bw()+
  theme(legend.position="none")+annotation_custom(dcLabel)
...
p5 <- ggplot() +
  geom_ribbon(data = allCovars[compAcSet_MP_all,],
            aes(x = date, ymax = predSd + pred, ymin = pmax(pred - predSd,0)), fill = "lightgray") +
  geom_area(data = allCovars[compAcSet_MP_all,], 
            aes(x = date, y = pmax(pred - predSd,0)), fill = "white") +
  geom_path(data = allCovars[compAcSet_MP_all,],
            aes(x= date, y=pred, color = Legend)) +
  geom_point(data = allCovars[compAcSet_MP_all,],
             aes(x = date, y = observations, color = DataGroup),alpha = 0.5) +
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "Date")+theme_bw()+
  theme(legend.position="none")+annotation_custom(mpLabel)
...
july2009_Ac_prediction_se = rasterize(myXY,july2009_AcOnly_prediction,july2009_Ac_se_predict_points$se.fit,na.rm = TRUE)
...
july2009_Vis_prediction_se = rasterize(myXY,july2009_VisOnly_prediction,july2009_Vis_se_predict_points$se.fit,na.rm = TRUE)
```
...
july2009_prediction_se = rasterize(myXY,july2009_prediction,july2009_joint_se_predict_points$se.fit,na.rm = TRUE)
...
png(paste(outDir,SP,'_summer2009maps_GAM.png',sep=''), width = 6, height = 7, units = 'in',res = 600)
...
JointMapSummer = tm_shape(july2009_prediction_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColor/2,title = "P(Encounter)",legend.reverse = TRUE)+ 
...
AcMapSummer = tm_shape(july2009_AcOnly_prediction_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColor/2,title = "P(Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
...
VisMapSummer = tm_shape(july2009_VisOnly_prediction_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColor/2,title = "P(Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
...
# plot best summer map for publication
png(paste(outDir,SP,'_summer2009maps_GAM_se.png',sep=''), width = 6, height = 7, units = 'in',res = 600)
...
JointMapSummer = tm_shape(july2009_prediction_se_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(P. Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
...
AcMapSummer = tm_shape(july2009_AcOnly_prediction_se_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(P. Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
...
VisMapSummer = tm_shape(july2009_VisOnly_prediction_se_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(P. Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
...
for (iM in 1:length(monthStr)){
  ...
  rasterImageFileName = paste0(savePath,'/climatology_predictions/', SP,'_',monthStr[iM],'GAM_mean_encounter_probability.tif')
