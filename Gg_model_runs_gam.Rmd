...
png(paste(outDir,SP,'_Joint_smooths_pres_abs.png',sep=''), res = 300, units = "in", width = 6.5, height = ceil(nTerms/3)*2)


par(mfrow = c(ceil(nTerms/3),3),mar = c(4,2,1,1))
for  (i in 1:nTerms) {
  plot(Joint_model,select = i,
       cex.lab = 1.3,cex.axis = 1.1,scale=0,xlab = varUnits[significantPvals_Joint[i]], ylab = "")
}
...
XEntropy$AcOnly_test_JointData <- weighted_logloss(round(pr$AcOnly_JointData$fit,
                                 ...
                                 as.vector(ones(length(joint_test_weightsG0),1)))
XEntropy$VisOnly_test_JointData <- weighted_logloss(round(pr$VisOnly_JointData$fit,
                                                          digits = 0),
                                 as.vector(transformedCovars.test$y_TF),
                                 as.vector(ones(length(joint_test_weightsG0),1)))
XEntropy$Joint_test <- weighted_logloss(round(pr$Joint_JointData$fit, digits = 0),
                                 as.vector(transformedCovars.test$y_TF),
                                 as.vector(ones(length(joint_test_weightsG0),1)))
