---
title: 'Kogia spp. habitat models: Neural Net'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
```{r dot plots, eval = FALSE, message = FALSE, results = 'hide'}
...
```{r transformed dot plots, eval = FALSE, message = FALSE, results = 'hide'}
...
```{r presence absence histograms, eval = FALSE, message = FALSE, results = 'hide'}
...
```{r Ac. only Model 1, results = 'hide', message = FALSE, eval = FALSE}
...
```{r other models 1, eval = FALSE, include = FALSE} 
## VISUAL ONLY
# put together the formula
...
```{r Ac. Only Model 2, results = 'hide', message = FALSE, eval = FALSE}
...
```{r Ac Only Model 3, results = 'hide', message = FALSE, eval = FALSE}
...
for (iM in 1:length(monthStr)){
  SST_climate <- scale(resample(raster(paste0(SST_ClimateDir,'/',
                    ...
                    covars_Joint_max.train[1]-covars_Joint_min.train[1])
  SSH_climate <- scale(resample(raster(paste0(SSH_ClimateDir,'/',
                                        'proj_SSH_',
  ...
  eddy_climate <- scale(resample(raster(paste0(eddy_ClimateDir,'/',
                                        'proj_posEddyDist_',
                                        monthStr[iM],'_climatology.tif')),predTemplate),
  ...
  log10_HYCOM_MAG_climate <- scale(log10(resample(raster(paste0(HYCOM_MAG_ClimateDir,'/',
                                        ...
                                        monthNum[iM],'_mean.img')),predTemplate)),
                                   center = covars_Joint_min.train[7],scale = 
                    covars_Joint_max.train[7]-covars_Joint_min.train[7])
