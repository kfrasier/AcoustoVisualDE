...
for (layerSize in layerSizeList){
  ...
  MSE$nn_AcOnly_test_allData[[AcCounter]] <- mean(
...
for (layerSize in layerSizeList){
  ...
  MSE$nn_VisOnly_test_allData[[modelCounter]] <- mean(
                                        abs(exp(log(covars_Joint_max.train['y'])*(Joint_test_scaled$y - 
...
# 
#   MSE$nn_Joint_train[[modelCounter]]  <- mean(
...
save(nn_Joint,MSE,pr,
     file = paste(outDir,SP,'_Joint_NN_density_noWeights.Rdata',sep=''))
```
...
load(paste0(outDir,SP,'_Joint_NN_density_noWeights.Rdata'))
```
...
#Visual on Visual
  pr$nn_VisOnly_test_VisData[[modelCounter]] <-(exp(log(covars_Joint_max.train['y'])*
...
# Visual on Acoustic
pr$nn_VisOnly_test_AcData[[modelCounter]] <-(exp(log(covars_Joint_max.train['y'])*
                                                     predict(nn_VisOnly[[modelCounter]],
                                                      AcOnly_test_scaled[,model1.indices],
                                                      na.action = na.omit)))-1
...
#Acoustic on Visual
pr$nn_AcOnly_test_VisData[[modelCounter]] <-(exp(log(covars_Joint_max.train['y'])*
...
pr$nn_AcOnly_test_AcData[[modelCounter]] <-(exp(log(covars_Joint_max.train['y'])*
                                                     predict(nn_AcOnly[[modelCounter]],
                                                      AcOnly_test_scaled[,model1.indices],
                                                      na.action = na.omit)))-1
...
p4_Ac <- ggplot() +
  ...
  geom_col(data = AcOnly_predictionSet,
             aes(x = date, y = obs_DC,color = Legend1),alpha = .5)+
  geom_line(data = AcOnly_predictionSet,
            aes(x=date, y=DC, color = Legend,ymax = myYmax)) +
  geom_line(data = AcOnly_predictionSet[AcOnly_predictionSet$date>="2013-1-1",],
            aes(x=date, y=DC, color = LegendTest, ymax = myYmax)) +
  scale_color_manual(values= c("#A0A0A0","#009900","#0000FF"))+
  labs(y = "", x = "")+theme_bw()+
...
p1 <- ggplot() +
  ...
  theme(legend.position=c(.90,.7),legend.title=element_blank(),
        ...
        plot.margin = unit(c(0,0,10,5), "pt"),
...
p2 <- ggplot() +
  ...
  theme(legend.position="none", axis.title.y = element_text(size = 8,),panel.spacing.y=unit(0, "lines"),plot.margin = unit(c(0,0,10,5), "pt"),axis.text.x = element_blank())+
  annotation_custom(gcLabel)
...
p4 <- ggplot() +
  ...
  scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual"),
                     values= c("grey46","gray34","deepskyblue","red2"))+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,10,5),"pt"),axis.text.x = element_blank())+coord_cartesian(ylim = c(0,40))+labs(y = " ")+
  annotation_custom(dcLabel)
...
p5 <- ggplot() +
  ...
  scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual"),
                     values= c("grey46","gray34","deepskyblue","red2"))+theme_bw()+
  theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,0,5),"pt"))+coord_cartesian(ylim = c(0,35))+
  annotation_custom(mpLabel)
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  ...
  addLayersControl(
    baseGroups = c('Acoustic July 2009','Visual July 2009','Vis. & Ac. Mean July 2009'),
...
maxColorUncert <- ceiling(max(c(predSTD_july2009_AcOnly_crop@data@max,
               ...
               predSTD_jan2009_VisOnly_crop@data@max))*100)/100
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  ...
  addLayersControl(
    baseGroups = c('2 nodes','4 nodes','6 nodes',
                   '8 nodes','10 nodes'),
    options = layersControlOptions(collapsed = FALSE)
  )
map
