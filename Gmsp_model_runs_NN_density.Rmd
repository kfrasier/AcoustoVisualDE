---
title: 'GoMx pilot whale habitat models: Density'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
load("E:/NASData/ModelData/Gmsp/GmspsightwTrunc_GU.Rdata")
...
for (layerSize in layerSizeList){
  ...
  MSE.nn_AcOnly_train[[AcCounter]] <- sum((AcOnly_train_scaled$yAcOnly - 
                                    pr.nn_AcOnly_train[[AcCounter]])^2)/
  ...
  MSE.nn_AcOnly_test[[AcCounter]] <- sum((AcOnly_test_scaled$yAcOnly - 
...
for (layerSize in layerSizeList){
  ...
  nn_VisOnly[[modelCounter]] <- avNNet(f.VisOnly_NN1, VisOnly_train_scaled,
                                       ...
                                       size = layerSize, 
                                       repeats = trainRepeats,linout = TRUE,
                                       na.action = na.omit,
                                       rang = 0.7,  
                                       decay = 0.0001, 
                                       maxit = 10000,
  ...
  pr.nn_Joint_train[[modelCounter]]  <- predict(nn_Joint[[modelCounter]],
                                         ...
                                         na.action=na.omit)
  MSE.nn_Joint_train[[modelCounter]]  <- sum(Joint_train_scaled$weightsG0*(Joint_train_scaled$y - 
  ...
  pr.nn_Joint_test[[modelCounter]]  <- predict(nn_Joint[[modelCounter]],
                                        ...
                                        na.action=na.omit)
  MSE.nn_Joint_test[[modelCounter]]  <- sum(Joint_test_scaled$weightsG0*
                                        (Joint_test_scaled$y - 
...
Models were compared using mean squared error to compare predicted and observed presence/absence in the test data.
...
![](E:/NASData/ModelData/Gmsp/Gmsp_exampleNetwork.png)
...
**Predicted and observed densities at passive acoustic sites using the acoustic-only model (Site order: MC, GC, DT):**
...
jan2009_VisOnly_prediction <- raster::predict(jan2009_rastersVis,
         nn_VisOnly[[best_VisOnly_ModelIndex]],na.action = na.pass)*covars_Joint_max.train['y']/10
...
for (iM in 1:length(monthStr)){
  climatePrediction[[iM]] <- raster::predict(raster_set[[iM]],nn_Joint[[best_Joint_ModelIndex]],
           na.action = na.pass)*covars_Joint_max.train['y']
  ...
  rasterImageFileName = paste0(savePath,'/climatology_predictions/', SP,'_',monthStr[iM],'NN_mean_density.tif')
