...
load('F:/NASData/ModelData/Gg/setup_info_Gg.Rdata')
load('F:/NASData/ModelData/Gg/GgMergedData.Rdata')
outDir <- file.path('F:/NASData/ModelData',SP,'newGAM_density//')
inDir <- file.path('F:/NASData/ModelData',SP,'//')
```
<br>
...
# save(gam_full_Joint,file = paste(outDir,SP,'_Joint_tw_GAMs_Density_noWeights.Rdata',sep=''))
```
...
# load(paste(outDir,SP,'_Joint_tw_GAMs_Density_noWeights.Rdata',sep=''))
```
...
RMSETable <- rbind(RMSE$AcOnly_test_VisData,RMSE$VisOnly_test_VisData,RMSE$AcOnly_test_AcData,RMSE$VisOnly_test_AcData)
...
rownames(RMSETable)<- c("Acoustic - Test Visual Data",
                       "Visual - Test Visual Data",
                       "Acoustic - Test Acoustic Data",
...
multiplot(p1_Vis, p2_Vis, p3_Vis, p4_Vis, cols=1)
dev.off()
```
...
```{r,fig.height = 10, fig.width = 7, message = FALSE, warning = FALSE, echo = FALSE}
p1 <- ggplot() +
  geom_rect(data =Vis_predictionSet, aes(xmin=allTheDays[731], xmax=allTheDays[1095], ymin=0, ymax=50), fill="gray85")+
  geom_col(data = Ac_predictionSet,
...
p3 <- ggplot() +
  geom_rect(data =Vis_predictionSet, aes(xmin=allTheDays[731], xmax=allTheDays[1095], ymin=0, ymax=100), fill="gray85")+
  geom_col(data = Ac_predictionSet,
             aes(x = allTheDays, y = DT_data, color = Legend1), alpha = 1,width = .1,size = .25,key_glyph = "rect")+
  geom_line(data = Ac_predictionSet,
             aes(x = allTheDays, y = DT_data_smooth, color = Legend), alpha = 1,width = .1,size = .25,key_glyph = "rect")+
  geom_line(data = Ac_predictionSet,
            aes(x=allTheDays, y=DT, color = LegendTest), size = .5,key_glyph = "rect") +
  geom_line(data = Vis_predictionSet,
            aes(x=allTheDays, y=DT, color = LegendTest),size = .5,key_glyph = "rect") +
  labs(y = "", x = NULL)+coord_cartesian(ylim = c(0,100))+  theme_bw()+
  scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual"),
                     values= c("grey46","gray34","deepskyblue","red2"),
                     guide = guide_legend(keyheight = unit(1, units = "mm") , keywidth = unit(2, units = "mm")))+
  theme(legend.position="none", axis.title.y = element_text(size = 8,),panel.spacing.y=unit(0, "lines"),plot.margin = unit(c(0,0,10,9), "pt"),axis.text.x = element_blank())+
...
p4 <- ggplot() +
  ...
  theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,10,12), "pt"),
        axis.text.x = element_blank())+
  annotation_custom(dcLabel)
png(paste(outDir,SP,'_Timeseries_Prediction_gam_Density.png',sep=''), width = 6.5, height = 4, units = "in", res=300)
...
predTemplate <-raster('F:/NASData/Eddy/RefRaster.tif')
map_proj <- crs(predTemplate)
crop_limits <- readOGR('F:/NASData/AcoustoVisualDE/Prediction_template/prediction_polygon.shp',verbose=FALSE)
...
jan2009mean_prediction_crop <- mask(jan2009mean,july2009_VisOnly_prediction)
july2009mean_prediction_crop <- mask(july2009mean,july2009_VisOnly_prediction)
...
GU2009Effort <- readOGR('F:/NASData/GU2009Effort/GU_Effort_Merge_clip_Project.shp',verbose=FALSE)
