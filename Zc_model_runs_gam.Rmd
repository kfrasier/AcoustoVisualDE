---
title: 'Cuvier''s beaked whale habitat models: GAMs'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
![](E:/NASData/ModelData/Zc/AcousticOnly_Zc_missingData.png)
<br> 
...
```{r Calculate weights, echo = FALSE}
...
weightsG0<- array(data = 1, dim = c(length(transformedCovars_VisOnly.train$fac1),1))
...
transformedCovars_VisOnly.train$weightsG0<-weightsG0
...
gam_full_AcOnly_TF$v01 <- gam(yAcOnly_TF ~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
...
```{r other models 1, eval = FALSE, include = TRUE, echo = FALSE} 

gam_full_VisOnly_TF$v01 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               ...
                               na.action = na.omit,family = binomial(),
                               weights = transformedCovars_VisOnly.train$weightsG0* 
                                 (Train_VisOnly.set$Weight/max(Train_VisOnly.set$Weight)))


gam_full_TF$v01 <- gam(y_TF~ s(SST, bs='ts', k=kVal) 
                        + s(SSH, bs='ts', k=kVal)
                        + s(HYCOM_UPVEL_50, bs='ts', k=kVal)
                        + s(EddyDist,bs='ts', k=kVal)                        
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal)
                        ...
                        weights = transformedCovars.train$weightsG0* 
                                 (mergedTrain.set$Weight/max(mergedTrain.set$Weight)),
                        na.action = na.omit,family = binomial())
```
...
gam_full_AcOnly_TF$v02 <- gam(yAcOnly_TF ~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
...
```{r other models 2, eval = FALSE, include = TRUE, echo = FALSE} 

gam_full_VisOnly_TF$v02 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(HYCOM_UPVEL_50,bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
                               + s(Neg_EddyDist, bs='ts', k=kVal)
...
gam_full_TF$v02 <- gam(y_TF~ s(SST, bs='ts', k=kVal) 
                        + s(SSH, bs='ts', k=kVal)
                        + s(HYCOM_UPVEL_50, bs='ts', k=kVal)
                        + s(Neg_EddyDist,bs='ts', k=kVal)                        
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal)
...
gam_full_AcOnly_TF$v03 <- gam(yAcOnly_TF ~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
...
```{r other models 3, eval = FALSE, include = TRUE, echo = FALSE} 

gam_full_VisOnly_TF$v03 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(HYCOM_UPVEL_50,bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
                               + s(Pos_EddyDist, bs='ts', k=kVal)
...
gam_full_TF$v03 <- gam(y_TF~ s(SST, bs='ts', k=kVal) 
                        + s(SSH, bs='ts', k=kVal)
                        + s(HYCOM_UPVEL_50, bs='ts', k=kVal)
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal)
                        + s(log10_HYCOM_MLD, bs='ts', k=kVal),
...
gam_full_AcOnly_TF$v04 <- gam(yAcOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
...
```{r Other models 4, eval = FALSE, include = TRUE, echo = FALSE}
gam_full_VisOnly_TF$v04 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, EddyDist, bs='ts', k=kVal)
                               + s(HYCOM_UPVEL_50,bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal),
...
gam_full_TF$v04 <- gam(y_TF~ s(SST, bs='ts', k=kVal) 
                        + s(HYCOM_UPVEL_50, bs='ts', k=kVal)
                        + s(SSH, EddyDist,  bs='ts', k=kVal)
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal)
                        + s(log10_HYCOM_MLD, bs='ts', k=kVal),
...
save(gam_full_TF,file = paste(outDir,SP,'_binomial_GAMs_ALL.Rdata',sep=''))
```
...
for (iMod in 1:length(gam_full_AcOnly_TF)){
  AC_model_AIC[iMod] <- AIC(gam_full_AcOnly_TF[[iMod]])
  cat('Model ', names(gam_full_AcOnly_TF[iMod]), ' AIC = ', AC_model_AIC[iMod], '\n')
}
```

```{r Best acoustic model, echo = FALSE}
best_AcOnly_model_index <- which.min(AC_model_AIC)
best_AcOnly_model <- gam_full_AcOnly_TF[[best_AcOnly_model_index]]
best_AcOnly_model_name <- names(gam_full_AcOnly_TF[best_AcOnly_model_index])
...
for (iMod in 1:length(gam_full_VisOnly_TF)){
  Vis_model_AIC[iMod] <- AIC(gam_full_VisOnly_TF[[iMod]])
  cat('Model ', names(gam_full_VisOnly_TF[iMod]), ' AIC = ', Vis_model_AIC[iMod], '\n')
}
```

```{r Best visual model, echo = FALSE}
best_VisOnly_model_index <- which.min(Vis_model_AIC)
best_VisOnly_model <- gam_full_VisOnly_TF[[best_VisOnly_model_index]]
best_VisOnly_model_name <- names(gam_full_VisOnly_TF[best_VisOnly_model_index])
...
for (iMod in 1:length(gam_full_TF)){
  model_AIC[iMod] <- AIC(gam_full_TF[[iMod]])
...
best_model <- gam_full_TF[[best_model_index]]
best_model_name <- names(gam_full_TF[best_model_index])
...
```{r best AC model, message=FALSE, results = 'hide', echo = FALSE, warning=FALSE}
AcOnly_gam_pruned_TF_best <- gam(yAcOnly_TF ~ s(SST, bs='ts', k=kVal)
                                      + s(SSH,bs='ts', k=kVal)
                                      + s(log10_CHL,bs='ts', k=kVal)
                                      + s(Pos_EddyDist,bs='ts', k=kVal)
                                      + s(HYCOM_SALIN_0,bs='ts', k=kVal),
...
summary(AcOnly_gam_pruned_TF_best)
plot(AcOnly_gam_pruned_TF_best,pages=1,cex.lab = 1.3,cex.axis = 1.1,scale=0)
save(AcOnly_gam_pruned_TF_best,file = paste(outDir,SP,'_best_AcOnly_binomial_model.Rdata',sep=''))

```
<br>

```{r AC best model residuals, echo = FALSE}
plot(residuals.gam(AcOnly_gam_pruned_TF_best))
...
```{r best Vis model, message=FALSE, results = 'hide', echo = FALSE, warning=FALSE}
VisOnly_gam_pruned_TF_best <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(HYCOM_UPVEL_50, bs='ts', k=kVal)
                               + s(Pos_EddyDist, bs='ts', k=kVal)
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal),
                               data = transformedCovars_VisOnly.train,
...
summary(VisOnly_gam_pruned_TF_best)
plot(VisOnly_gam_pruned_TF_best,pages=1,cex.lab = 1.3,cex.axis = 1.1,scale=0)
save(VisOnly_gam_pruned_TF_best,file = paste(outDir,SP,'_best_VisOnly_binomial_model.Rdata',sep=''))

```

```{r Vis best model residuals, echo = FALSE}
plot(residuals.gam(VisOnly_gam_pruned_TF_best))
...
```{r unpruned model summary, echo  = FALSE}
summary(best_model)
```
<br>
...
```{r best model, echo = FALSE, message=FALSE, results = 'hide', warning=FALSE}
gam_pruned_TF_best <- gam(y_TF~ s(SST, bs='ts', k=kVal) 
                        + s(SSH, bs='ts', k=kVal)
                        + s(EddyDist, bs='ts', k=kVal)
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal)
                        + s(log10_HYCOM_MLD,bs='ts', k=kVal),
...
summary(gam_pruned_TF_best)
plot(gam_pruned_TF_best,pages=1,cex.lab = 1.3,cex.axis = 1.1,scale=0)
save(gam_pruned_TF_best,file = paste(outDir,SP,'_best_joint_binomial_model.Rdata',sep=''))

```
<br>

```{r best model residuals, echo = FALSE}
plot(residuals.gam(gam_pruned_TF_best))
...
```{r Ac Temporal Predict, echo = FALSE}
...
predAcOnly_MC <- predict.gam(AcOnly_gam_pruned_TF_best,
                             transformedCovars_AcOnly.test[compAcSet_MC,],
                             type = 'response',na.action = na.pass)
predAcOnly_GC <- predict.gam(AcOnly_gam_pruned_TF_best,
                             transformedCovars_AcOnly.test[compAcSet_GC,],
                             type = 'response',na.action = na.pass)
predAcOnly_DT <- predict.gam(AcOnly_gam_pruned_TF_best,
...
predVisOnly_MC <- predict.gam(VisOnly_gam_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_MC,],
                              type = 'response',na.action = na.pass)
predVisOnly_GC <- predict.gam(VisOnly_gam_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_GC,],
                              type = 'response',na.action = na.pass)
predVisOnly_DT <- predict.gam(VisOnly_gam_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_DT,],
                              type = 'response',na.action = na.pass)
...
pred_MC <- predict.gam(gam_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_MC,],
                              type = 'response',na.action = na.pass)
pred_GC <- predict.gam(gam_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_GC,],
                              type = 'response',na.action = na.pass)
pred_DT <- predict.gam(gam_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_DT,],
                              type = 'response',na.action = na.pass)
...
jan2009_VisOnly_prediction <- raster::predict(jan2009_rasters,VisOnly_gam_pruned_TF_best,
            type = 'response',na.action = na.pass)
            
july2009_VisOnly_prediction <- raster::predict(july2009_rasters,VisOnly_gam_pruned_TF_best,
            type = 'response',na.action = na.pass)    
```
...
jan2009_prediction <- raster::predict(jan2009_rasters,gam_pruned_TF_best,
            type = 'response',na.action = na.pass)
            
july2009_prediction <- raster::predict(july2009_rasters,gam_pruned_TF_best,
            type = 'response',na.action = na.pass)    
```
<br>
...
```{r wrangle projections, message = FALSE, warning = FALSE, results = 'hide', echo = FALSE}
...
```{r build climatology raster-bricks, eval = FALSE, include = FALSE, echo = FALSE}
...
for (iM in 1:length(monthStr)){
  climatePrediction[[iM]] <- raster::predict(raster_set[[iM]],gam_pruned_TF_best,
            type = 'response',na.action = na.pass)
