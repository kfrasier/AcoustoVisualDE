...
```{r plot timeseries, message = FALSE, eval = FALSE}
...
```{r pres abs plots, message = FALSE, eval = FALSE}
# without transform
...
yAcOnly <- round(Train_AcOnly.set$Density)
yVisOnly <- round(Train_VisOnly.set$Density)
y <- as.logical(mergedTrain.set$Density)

# initialize empty structure for model storage

gamm_full_AcOnly<-NULL
glmm_full_VisOnly<-NULL
glmm_full<-NULL
```
...
gamm_full_AcOnly$v01 <- glmmTMB(yAcOnly~ SST+SSH+log10_CHL+EddyDist+log10_HYCOM_MAG_0+
                               log10_FrontDist_Cayula+HYCOM_SALIN_0+log10_HYCOM_MLD,
                               ziformula = ~fac1,
                               family = poisson(),
                               data = transformedCovars_AcOnly.train)

test1 <- glmmPQL(yAcOnly~ SST+SSH+log10_CHL+EddyDist+log10_HYCOM_MAG_0+
                               log10_FrontDist_Cayula+HYCOM_SALIN_0+log10_HYCOM_MLD,
                               random = ~1|fac1,
                               family = poisson,
                               na.action = na.omit,
