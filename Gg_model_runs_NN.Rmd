---
title: "GoMx Risso's dolphin habitat models: Presence/absence with neural nets"
author: 
...
outDir <- file.path('E:/NASData/ModelData',SP,'10000_predAbs//')
...
varUnits <- c("SST (C)","SSH (m)",expression("Chlorophyll A (mg/m"^3*")"),"MLD (m)", "Salinity (ppm)",                         "Current Magnitude (m/sec)", "Upwelling (m/sec)",
...
```{r plot weights, echo = FALSE, message = FALSE, results = 'hide'}
png(paste(outDir,SP,'_caseWeights_NN.png',sep=''), width = 4, height = 4, units = 'in',res = 300)
#  par(mar = c(5.1, 4.1, 4.1, 3.1))
plot(joint_train_weightsG0,ylim =c(0,1 ),ylab='Case Weight',xlab = 'Case Index',cex = 1)
abline(v = 2016,col = "darkgrey",lty = 2, lwd = 3)

dev.off()
wghtFig <-paste0(outDir,SP,'_caseWeights_NN.png')
```
...
for (layerSize in layerSizeList){
  ...
  pr$nn_AcOnly_train[[AcCounter]] <- predict(nn_AcOnly[[AcCounter]],
                                             ...
                                             na.action = na.omit)
  MSE$nn_AcOnly_train[[AcCounter]] <- mean(abs(AcOnly_train_scaled$yAcOnly_TF - 
  ...
  MSE$nn_AcOnly_test[[AcCounter]] <- mean(abs(AcOnly_test_scaled$yAcOnly_TF - 
                         pr$nn_AcOnly_test[[AcCounter]]),na.rm = TRUE)
  
  ## how well does the model predict joint test data?
  pr$nn_AcOnly_test_JointData[[AcCounter]] <- predict(nn_AcOnly[[AcCounter]],
                                                Joint_test_scaled[,model1.indices],
                                                na.action = na.omit)

  MSE$nn_AcOnly_test_JointData[[AcCounter]] <- mean(Joint_test_scaled$weightsG0*
                                        abs(Joint_test_scaled$y_TF - 
                                        pr$nn_AcOnly_test_JointData[[AcCounter]]),
                                        na.rm = TRUE)
...
for (layerSize in layerSizeList){
  ...
  MSE$nn_VisOnly_train[[modelCounter]] <- mean(VisOnly_train_scaled$weightsG0*
                                        abs(VisOnly_train_scaled$yVisOnly_TF - 
                                        pr$nn_VisOnly_train[[modelCounter]]),
                                        na.rm = TRUE)
  ...
  MSE$nn_VisOnly_test[[modelCounter]] <- mean(VisOnly_test_scaled$weightsG0*
                                        abs(VisOnly_test_scaled$yVisOnly_TF - 
                                        pr$nn_VisOnly_test[[modelCounter]]),
                                        na.rm = TRUE)
  
  ## how well does the model predict joint test data?
  pr$nn_VisOnly_test_JointData[[modelCounter]] <- predict(nn_VisOnly[[modelCounter]],
                                                Joint_test_scaled[,model1.indices],
                                                na.action = na.omit)

  MSE$nn_VisOnly_test_JointData[[modelCounter]] <- mean(Joint_test_scaled$weightsG0*
                                        abs(Joint_test_scaled$y_TF - 
                                        pr$nn_VisOnly_test_JointData[[modelCounter]]),
                                        na.rm = TRUE)
  ...
  MSE$nn_Joint_train[[modelCounter]]  <- mean(Joint_train_scaled$weightsG0*
                                         abs(Joint_train_scaled$y_TF - 
                                         pr$nn_Joint_train[[modelCounter]]),
                                         na.rm = TRUE)
...
load(paste0(outDir,SP,'_AcOnly_NN.Rdata'))
load(paste0(outDir,SP,'_VisOnly_NN.Rdata'))
load(paste0(outDir,SP,'_Joint_NN.Rdata'))
```
...
XEntropy <- NULL
XEntropy_unweighted <- NULL
for (iM in 1: length(nn_AcOnly)){
  XEntropy$AcOnly_train[[iM]] <- LogLoss(round(pr$nn_AcOnly_train[[iM]],digits = 4),
                                         AcOnly_train_scaled$yAcOnly_TF)
  XEntropy$AcOnly_test[[iM]] <- LogLoss(round(pr$nn_AcOnly_test[[iM]],digits = 4),
                                        AcOnly_test_scaled$yAcOnly_TF)
  XEntropy$AcOnly_test_JointData[[iM]] <- weighted_logloss(round(pr$nn_AcOnly_test_JointData[[iM]],digits = 4),
  ...
  XEntropy$VisOnly_test[[iM]] <- weighted_logloss(round(pr$nn_VisOnly_test[[iM]],digits = 4),
                                         VisOnly_test_scaled$yVisOnly_TF,
                                         VisOnly_test_scaled$weightsG0)
  XEntropy$VisOnly_test_JointData[[iM]] <- weighted_logloss(round(pr$nn_VisOnly_test_JointData[[iM]],digits = 4),
                                        Joint_test_scaled$y_TF,
                                        Joint_test_scaled$weightsG0)
  
  XEntropy$Joint_train[[iM]] <- weighted_logloss(round(pr$nn_Joint_train[[iM]], digits = 4),
                                         Joint_train_scaled$y_TF,
                                         Joint_train_scaled$weightsG0)
  XEntropy$Joint_test[[iM]] <- weighted_logloss(round(pr$nn_Joint_test[[iM]], digits = 4),
                                         Joint_test_scaled$y_TF,
                                         Joint_test_scaled$weightsG0)
  
  XEntropy_unweighted$AcOnly_test_JointData[[iM]] <- LogLoss(round(pr$nn_AcOnly_test_JointData[[iM]],
                                                                 digits = 4),
                                        Joint_test_scaled$y_TF)

  XEntropy_unweighted$VisOnly_test_JointData[[iM]] <- LogLoss(round(pr$nn_VisOnly_test_JointData[[iM]],
                                                                  digits = 4),
                                        Joint_test_scaled$y_TF)
  
  XEntropy_unweighted$Joint_test[[iM]] <- LogLoss(round(pr$nn_Joint_test[[iM]], digits = 4),
                                         Joint_test_scaled$y_TF)
...
KSStat <- NULL
for (iM in 1: length(nn_AcOnly)){
  KSStat$AcOnly_train[[iM]] <- KS_Stat(round(pr$nn_AcOnly_train[[iM]],digits = 4),
                                       AcOnly_train_scaled$yAcOnly_TF)
  KSStat$AcOnly_test[[iM]] <- KS_Stat(round(pr$nn_AcOnly_test[[iM]],digits = 4),
                                      AcOnly_test_scaled$yAcOnly_TF)
  KSStat$VisOnly_train[[iM]] <- KS_Stat(round(pr$nn_VisOnly_train[[iM]],digits = 4),
                                        VisOnly_train_scaled$yVisOnly_TF)
  KSStat$VisOnly_test[[iM]] <- KS_Stat(round(pr$nn_VisOnly_test[[iM]],digits = 4),
                                       VisOnly_test_scaled$yVisOnly_TF)
  KSStat$Joint_train[[iM]] <- KS_Stat(round(pr$nn_Joint_train[[iM]],digits = 4),
                                      Joint_train_scaled$y_TF)
  KSStat$Joint_test[[iM]] <- KS_Stat(round(pr$nn_Joint_test[[iM]],digits = 4),
                                     Joint_test_scaled$y_TF)
}
...
#                  MSE$nn_AcOnly_test_allData,MSE$nn_VisOnly_test_allData)
...
XEntropyTable<- rbind( XEntropy$AcOnly_test_JointData,
                       XEntropy$VisOnly_test_JointData,
                       XEntropy$Joint_test,
                       XEntropy_unweighted$AcOnly_test_JointData,
                       XEntropy_unweighted$VisOnly_test_JointData,
                       XEntropy_unweighted$Joint_test)

colnames(XEntropyTable)<- layerSizeList
rownames(XEntropyTable)<- c("Acoustic - Weighted","Visual - Weighted",
                       "Joint - Weighted","Acoustic - Unweighted",
                       "Visual - Unweighted","Joint - Unweighted")
...
best_AcOnly_ModelIndex <-which.min(XEntropy$AcOnly_test_JointData)
print(paste(c('Best acoustic model has', layerSizeList[best_AcOnly_ModelIndex], 
              'nodes.'),collapse = " "))
best_VisOnly_ModelIndex <-which.min(XEntropy$VisOnly_test_JointData)
...
For the best model in each category, the importance of each input variable was
calculated across the 50 model iterations.
...
mean_var_importance$AcOnly_CV <- AcOnly_node_cv
mean_var_importance$VisOnly <- VisOnly_node_mean
mean_var_importance$VisOnly_CV <- VisOnly_node_cv
mean_var_importance$Joint <- Joint_node_mean
mean_var_importance$Joint_CV <- Joint_node_cv
...
png(paste(outDir,SP,'_exampleNetwork_AcOnly_PresAbs.png',sep=''), width = 1200, height = 600)
par(mar = c(5.1, 4.1, 4.1, 3.1))
plot.nnet(nn_AcOnly[[best_AcOnly_ModelIndex]]$model[[1]],x.lab = n2, 
          y.lab = "P(encounter)",bias = F,cex.val=1, neg.col = 'red', circle.col = 'gray')
dev.off()

png(paste(outDir,SP,'_exampleNetwork_VisOnly_PresAbs.png',sep=''), width = 1200, height = 600)
par(mar = c(5.1, 4.1, 4.1, 3.1))
plot.nnet(nn_VisOnly[[best_VisOnly_ModelIndex]]$model[[1]],x.lab = n2, 
          y.lab = "P(encounter)",bias = F,cex.val=1,neg.col = 'red', circle.col = 'gray')
dev.off()

png(paste(outDir,SP,'_exampleNetwork_Joint_PresAbs.png',sep=''), width = 1200, height = 600)
par(mar = c(5.1, 4.1, 4.1, 3.1))
plot.nnet(nn_Joint[[best_Joint_ModelIndex]]$model[[1]],x.lab = n2, 
          y.lab = "P(encounter)",bias = F,cex.val=1, neg.col = 'red', circle.col = 'gray')
dev.off()
egNetFig <-paste0(outDir,SP,'_exampleNetwork_Joint_PresAbs.png')


n2 = varUnits[n[model1.indices]]
n3 = c("SST","SSH","CHL","MLD","SAL","CUR","Upwell","-Eddy","+Eddy")
tiff(paste(outDir,SP,'_exampleNetwork_Joint_presAbs.tiff',sep=''), res = 300,width = 4.75, height = 4, units = "in")
...
plot.nnet(nn_Joint[[best_Joint_ModelIndex]]$model[[bestNetwork]],x.lab = n3, 
          y.lab = "P(E)",bias = F,cex=1.3,circle.col='gray',bord.col = 'darkgray', 
          neg.col = 'red',node.labs = FALSE,circle.cex = 3, alpha.val =.8, max.sp = TRUE)
dev.off()
egNetFig <-paste0(outDir,SP,'_exampleNetwork_Joint_presAbs.tiff')
```
***Example network:***
...
AcOnly_predictionSet$predSd_MC[m1] <- rowSds(data.matrix(predVar_MC_DF))
AcOnly_predictionSet$yTF_MC <- NA
AcOnly_predictionSet$yTF_MC[m1]<- (Test_AcOnly.set$Density[compAcSet_MC]>0)*1
...
AcOnly_predictionSet$yTF_GC <- NA
AcOnly_predictionSet$yTF_GC[m2]<- (Test_AcOnly.set$Density[compAcSet_GC]>0)*1
...
AcOnly_predictionSet$yTF_DT <- NA
AcOnly_predictionSet$yTF_DT[m3]<- (Test_AcOnly.set$Density[compAcSet_DT]>0)*1
...
AcOnly_predictionSet$predSd_DC[m4] <- rowSds(data.matrix(predVar_DC_DF))
AcOnly_predictionSet$yTF_DC <- NA
AcOnly_predictionSet$yTF_DC[m4]<- (Test_AcOnly.set$Density[compAcSet_DC]>0)*1
...
AcOnly_predictionSet$predSd_MP[m5] <- rowSds(data.matrix(predVar_MP_DF))
AcOnly_predictionSet$yTF_MP <- NA
AcOnly_predictionSet$yTF_MP[m5]<- (Test_AcOnly.set$Density[compAcSet_MP]>0)*1
...
p4_Ac <- ggplot() +
  ...
  geom_point(data = AcOnly_predictionSet,
             aes(x = date, y = yTF_DC, color = Legend1),alpha = 0.5) +
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "")+theme_bw()+
...
p5_Ac <- ggplot() +
  ...
  geom_point(data = AcOnly_predictionSet,
             aes(x = date, y = yTF_MP, color = Legend1),alpha = 0.5) +
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "Date")+theme_bw()+
...
predSTD_jan2009_AcOnly_crop <- calc(stack(perModel_jan2009_AcOnly_crop),fun=std_err)
predSTD_july2009_AcOnly_crop <- calc(stack(perModel_july2009_AcOnly_crop),fun=std_err)
...
predSTD_jan2009_VisOnly_crop <- calc(stack(perModel_jan2009_VisOnly_crop),fun=std_err)

predSTD_july2009_VisOnly_crop <- calc(stack(perModel_july2009_VisOnly_crop),fun=std_err)
...
predSTD_jan2009_Joint_crop <- calc(stack(perModel_jan2009_Joint_crop),fun=std_err)
predSTD_july2009_Joint_crop <- calc(stack(perModel_july2009_Joint_crop),fun=std_err)
```
...
```{r plot best summer map, echo = FALSE, message = FALSE, results = 'hide',warning = FALSE, eval = TRUE}
...
JointMapSummer = tm_shape(july2009_prediction_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSummer/2,title = "P(Encounter)",legend.reverse = TRUE)+ 
...
<br>  
```{r plot best summer uncert map, echo = FALSE, message = FALSE, results = 'hide',warning = FALSE, eval = TRUE}
# plot best summer map for publication
png(paste(outDir,SP,'_summer2009maps_NN_uncert.png',sep=''), width = 6, height = 7, units = 'in',res = 600)
...
harpShape = SpatialPoints(HARPsites[c(1,8,14),c(2,1)],proj4string =  
                            crs(GU2009EffortLines))
crop_limits_Reproj = spTransform(crop_limits_proj,crs(GU2009EffortLines))
maxColor2 <- ceiling(max(c(predSTD_july2009_AcOnly_crop@data@max,
               predSTD_july2009_VisOnly_crop@data@max,
               predSTD_july2009_Joint_crop@data@max))*100)/100

JointMapSummerUncert = tm_shape(predSTD_july2009_Joint_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSummer/2,title = "SE(P. Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
...
AcMapSummerUncert = tm_shape(predSTD_july2009_AcOnly_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSummer/2,title = "SE(P. Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
...
VisMapSummerUncert = tm_shape(predSTD_july2009_VisOnly_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSummer/2,title = "SE(P. Encounter)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
 ...
 tmap_arrange(AcMapSummerUncert,VisMapSummerUncert,JointMapSummerUncert,ncol = 1, nrow = 3, outer.margins = 0.05)
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  ...
  addLegend(pal = palSummer2, values = c(0,.7),
    title = 'P(encounter)',position = "bottomleft") %>%
...
```{r plot best summer map hidden layer, echo = FALSE, message = FALSE, results = 'hide',warning = FALSE}
# plot best summer map for publication
png(paste(outDir,SP,'_summer2009maps_NN_HiddenLayerEffect.png',sep=''), width = 6, height = 4, units = 'in',res = 600)
...
crop_limits_Reproj = spTransform(crop_limits_proj,crs(GU2009EffortLines))
maxColorSummer = .7
JointMap1 = tm_shape(july2009_prediction_hidden[[1]])+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSummer/2,title = "P(Encounter)",legend.reverse = TRUE)+ tm_layout(panel.labels="4 Nodes",panel.label.size=.6)+
  tm_legend(legend.title.size = .9,scale = .8,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.3, alpha = 1,shape = 24)+
  tm_shape(sightShape)+tm_symbols(col = "black", border.col = "white",shape=21,size = 0.3, alpha = .75)
...
JointMap3 = tm_shape(july2009_prediction_hidden[[3]])+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSummer/2,legend.show = FALSE)+ tm_layout(panel.labels="8 Nodes",panel.label.size=.6)+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
