...
```{r map inputs, warning = FALSE, include = FALSE}
...
```{r plot timeseries, message = FALSE, include = FALSE}
...
```{r remove outliers, message = FALSE, results = 'hide', include = FALSE}
...
```{r dot plots, eval = FALSE, message = FALSE, results = 'hide', include = FALSE}
...
```{r correlation plots, eval = TRUE, message = FALSE, warning = FALSE, results = 'hide', include = FALSE}
...
```{r transform covars, message = FALSE, results = 'hide', include = FALSE}
# covarList2 <- c("SST","SSH","CHL","HYCOM_MLD",
#                 "HYCOM_SALIN_0","HYCOM_DIR_0",
#                 "HYCOM_MAG_0",
#                 "HYCOM_UPVEL_50","FrontDist_Cayula",
...
```{r transformed dot plots, eval = FALSE, message = FALSE, results = 'hide', include = FALSE}
...
```{r presence absence histograms, eval = FALSE, message = FALSE, results = 'hide', include = FALSE}
...
* Acoustic data have a trucation distance  of 0.7km for kogia(95% of group detections occur within this rance), for a total monitoring area of $(0.7 km)^{2} \pi$, 
...
```{r Calculate weights, include = FALSE, eval = FALSE}
...
```{r load starting data, include = FALSE} 
...
```{r Set up parallel cluster, echo = FALSE, eval = FALSE}
...
```{r build climatology raster-bricks, eval = FALSE, echo=FALSE}
...
for (iM in 1:length(monthStr)){
  ...
  pos_eddy_climate <- scale(resample(raster(paste0(eddy_ClimateDir,'/','proj_posEddyDist_',
                     monthStr[iM],'_climatology.tif')),predTemplate),
                     center = covars_Joint_min.train[11],scale = 
                     covars_Joint_max.train[10]-covars_Joint_min.train[11])
  
  neg_eddy_climate <- scale(resample(raster(paste0(eddy_ClimateDir,'/','proj_negEddyDist_',
                     monthStr[iM],'_climatology.tif')),predTemplate),
                     center = covars_Joint_min.train[12],scale = 
                     covars_Joint_max.train[10]-covars_Joint_min.train[12])  
  ...
  raster_set[[iM]] <- brick(SST_climate,SSH_climate,log10_CHL_climate,eddy_climate,log10_HYCOM_MAG_climate,
                         ...
                         neg_eddy_climate,pos_eddy_climate) 
