...
* Acoustic data have a trucation distance  of 1.9km for Risso's (95% of detections occur within this range), for a total monitoring area of $(1.9 km)^{2} \pi = 11.3 km^{2}$, 
...
* Visual trucation distance for Risso's is estimated as 5.3 km (see figure below) from these data. ESW is multiplied by transect segment length (usually ~10km) and doubled to estimate total area swept.


<br>  

\[\text{Acoustic to Visual ratio} = \frac{24 hrs \cdot (1.9 km)^{2} \pi}{(\text{Segment length}/18.52 km/hr) \cdot 5.3 km \cdot \text{Segment length} \cdot 2} = 272.18/61.56 \approx 4.5:1
\]
...
```{r Calculate weights, echo = FALSE, eval = FALSE}
...
# jan2009_VisOnly_prediction_SE_proj <-  projectRaster(jan2009_VisOnly_prediction_SE, crs=map_proj)
