...
* Visual estimated strip width (ESW) varies depending on the conditions. ESW for each sighting is multiplied by transect segment length (usually ~10km) and doubled to estimate total area swept.
...
```{r Other models 04, eval = FALSE, include = TRUE}
...
```{r Joint Model Spatial Predict, message=FALSE}
...
map <- leaflet() %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  ...
  addLegend(pal = pal, values = c(0,.4),
