---
title: 'GoMx Stenellid dolphin habitat models'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
# get rid of NA density values
noNaAcOnlyDensity <- which(!is.na(AcOnlySegments$Density))
AcOnlySegments<-AcOnlySegments[noNaAcOnlyDensity,]

noNaVisOnlyDensity <- which(!is.na(VisOnlySegments$Density))
VisOnlySegments<-VisOnlySegments[noNaVisOnlyDensity,]

noNaJointDensity <- which(!is.na(mergedSegments$Density))
mergedSegments<-mergedSegments[noNaJointDensity,]
...
```{r presence absence histograms, eval = FALSE, message = FALSE, results = 'hide', echo = FALSE}
...
The visual data represent wether or not stenellid dolphins were seen during each transect segment. The probability of missing a sighting of risso's dolphin was estimated as 
\[P_{V}(detect|present) = \mu_{det} * g0\ = 0.29 * 0.81 = 0.23\] 
...
The acoustic data represent presence or absence of stenellid dolphins detections in one day bins. Given that a group of dolphins is present near the sensor, the probability of detecting them in a 5 minute period is estimated at 0.65, therefore the probability of missing is 1-0.65 = 0.35. Given that animals were present, the probability of missing a group for a full day is estimated as
...
yAcOnlySqrt <- sqrt(yAcOnly)


load("E:/NASData/ModelData/Ssp/SspsightwTrunc_GU.Rdata")
visDetProb <- detFun[[bestModelIdx]]$fitted[1]

transformedCovars_AcOnly.train$yAcOnly_TF<-(Train_AcOnly.set$Density)*1
transformedCovars_VisOnly.train$yVisOnly_TF<-(Train_VisOnly.set$Density)*1
transformedCovars.train$y_TF<-(mergedTrain.set$Density)*1
transformedCovars_AcOnly.train$yAcOnlySqrt <-sqrt((Train_AcOnly.set$Density)*1)
transformedCovars_VisOnly.train$yVisOnlySqrt <-sqrt((Train_VisOnly.set$Density)*1)
transformedCovars.train$ySqrt <-sqrt((mergedTrain.set$Density)*1)

transformedCovars_AcOnly.test$yAcOnly_TF<-(Test_AcOnly.set$Density)*1
transformedCovars_VisOnly.test$yVisOnly_TF<-(Test_VisOnly.set$Density)*1
transformedCovars.test$y_TF<- (mergedTest.set$Density)*1
transformedCovars_AcOnly.test$yAcOnlySqrt <-sqrt((Test_AcOnly.set$Density)*1)
transformedCovars_VisOnly.test$yVisOnlySqrt <-sqrt((Test_VisOnly.set$Density)*1)
transformedCovars.test$ySqrt <-sqrt((mergedTest.set$Density)*1)
...
# Remove NaNs
goodData_Ac <- which(!is.na(rowSums(transformedCovars_AcOnly.train[,c(1:12)])))
AcOnly.train.NoNa <- transformedCovars_AcOnly.train[goodData_Ac,]

goodData_Vis <- which(!is.na(rowSums(transformedCovars_VisOnly.train[,c(1:12)])))
VisOnly.train.NoNa <- transformedCovars_VisOnly.train[goodData_Vis,]

goodData_Joint <- which(!is.na(rowSums(transformedCovars.train[,c(1:12)])))
Joint.train.NoNa <- transformedCovars.train[goodData_Joint,]


goodData_Ac_test <- which(!is.na(rowSums(transformedCovars_AcOnly.test[,c(1:12)])))
AcOnly.test.NoNa <- transformedCovars_AcOnly.test[goodData_Ac_test,]

goodData_Vis_test <- which(!is.na(rowSums(transformedCovars_VisOnly.test[,c(1:12)])))
VisOnly.test.NoNa <- transformedCovars_VisOnly.test[goodData_Vis_test,]

goodData_Joint_test <- which(!is.na(rowSums(transformedCovars.test[,c(1:12)])))
...
covars_AcOnly_max.train <- apply(AcOnly.train.NoNa[,c(1:15,17,18)], 2, max, na.rm = TRUE) 
covars_AcOnly_min.train <- apply(AcOnly.train.NoNa[,c(1:15,17,18)], 2, min, na.rm = TRUE)
AcOnly_train_scaled <- as.data.frame(scale(AcOnly.train.NoNa[,c(1:15,17,18)], 
                          center = covars_AcOnly_min.train, 
                          scale = covars_AcOnly_max.train-covars_AcOnly_min.train))
AcOnly_train_scaled$date <- AcOnly.train.NoNa$date

# Scale Vis only training data for the NN
covars_VisOnly_max.train <- apply(VisOnly.train.NoNa[,c(1:15,17,18)], 2, max, na.rm = TRUE) 
covars_VisOnly_min.train <- apply(VisOnly.train.NoNa[,c(1:15,17,18)], 2, min, na.rm = TRUE)
VisOnly_train_scaled <- as.data.frame(scale(VisOnly.train.NoNa[,c(1:15,17,18)], 
...
VisOnly_train_scaled$date <- VisOnly.train.NoNa$date
 
# Scale Joint training data for the NN
covars_Joint_max.train <- apply(Joint.train.NoNa[,c(1:15,17,18)], 2, max, na.rm = TRUE) 
covars_Joint_min.train <- apply(Joint.train.NoNa[,c(1:15,17,18)], 2, min, na.rm = TRUE)
Joint_train_scaled <- as.data.frame(scale(Joint.train.NoNa[,c(1:15,17,18)], 
...
Joint_train_scaled$date <- Joint.train.NoNa$date

# Scale Ac only test data for the NN
covars_AcOnly_max.test <- apply(AcOnly.test.NoNa[,c(1:15,17,18)], 2, max, na.rm = TRUE) 
covars_AcOnly_min.test <- apply(AcOnly.test.NoNa[,c(1:15,17,18)], 2, min, na.rm = TRUE)
AcOnly_test_scaled <- as.data.frame(scale(AcOnly.test.NoNa[,c(1:15,17,18)], 
                       center = covars_AcOnly_min.test,
                       scale = covars_AcOnly_max.test-covars_AcOnly_min.test))
AcOnly_test_scaled$date <- AcOnly.test.NoNa$date

# Scale Vis only test data for the NN
covars_VisOnly_max.test <- apply(VisOnly.test.NoNa[,c(1:15,17,18)], 2, max, na.rm = TRUE) 
covars_VisOnly_min.test <- apply(VisOnly.test.NoNa[,c(1:15,17,18)], 2, min, na.rm = TRUE)
VisOnly_test_scaled <- as.data.frame(scale(VisOnly.test.NoNa[,c(1:15,17,18)], 
...
VisOnly_test_scaled$date <- VisOnly.test.NoNa$date

# Scale Joint test data for the NN
covars_Joint_max.test <- apply(Joint.test.NoNa[,c(1:15,17,18)], 2, max, na.rm = TRUE) 
covars_Joint_min.test <- apply(Joint.test.NoNa[,c(1:15,17,18)], 2, min, na.rm = TRUE)
Joint_test_scaled <- as.data.frame(scale(Joint.test.NoNa[,c(1:15,17,18)], 
...
Joint_test_scaled$date <- Joint.test.NoNa$date
...
MSE.nn_AcOnly_train2<-NULL
MSE.nn_VisOnly_train2<-NULL
MSE.nn_Joint_train2<-NULL

MSE.nn_AcOnly_test<-NULL
MSE.nn_VisOnly_test<-NULL
MSE.nn_Joint_test<-NULL
MSE.nn_AcOnly_test2<-NULL
MSE.nn_VisOnly_test2<-NULL
MSE.nn_Joint_test2<-NULL
...
## ACOUSTIC ONLY
AcCounter <- 0
f.AcOnly_NN1 <- as.formula(paste("yAcOnlySqrt ~", paste(n[model1.indices], collapse = " + ")))
...
for (layerSize in layerSizeList){
  ...
  MSE.nn_AcOnly_train[[AcCounter]] <- sum((AcOnly_train_scaled$yAcOnlySqrt -
        pr.nn_AcOnly_train[[AcCounter]])^2)/nrow(AcOnly_train_scaled[model1.indices])
  
  MSE.nn_AcOnly_train2[[AcCounter]] <- sum(((AcOnly_train_scaled$yAcOnlySqrt^2) - 
                                        (pr.nn_AcOnly_train[[AcCounter]]^2)^2))/
                                        nrow(AcOnly_train_scaled[model1.indices])
  ...
  MSE.nn_AcOnly_test[[AcCounter]] <- sum((AcOnly_test_scaled$yAcOnlySqrt - 
                                   pr.nn_AcOnly_test[[AcCounter]])^2)/
                                   nrow(AcOnly_test_scaled[model1.indices])
  
  MSE.nn_AcOnly_test2[[AcCounter]] <- sum(((AcOnly_test_scaled$yAcOnlySqrt^2) - 
                                        (pr.nn_AcOnly_test[[AcCounter]]^2)^2))/
                                        nrow(AcOnly_test_scaled[model1.indices])
...
modelCounter <- 0
# put together the formula
f.VisOnly_NN1 <- as.formula(paste("yVisOnlySqrt ~", paste(n[model1.indices], collapse = " + ")))
f.Joint_NN1 <- as.formula(paste("ySqrt ~", paste(n[model1.indices], collapse = " + ")))
for (layerSize in layerSizeList){
  ...
  pr.nn_VisOnly_trainTemp <-predict(nn_VisOnly[[modelCounter]],VisOnly_train_scaled[,model1.indices])
  
  MSE.nn_VisOnly_train[[modelCounter]] <- sum(VisOnly_train_scaled$weightsG0*
                                                (VisOnly_train_scaled$yVisOnlySqrt - 
                                                pr.nn_VisOnly_train[[modelCounter]])^2)/
                                                nrow(VisOnly_train_scaled[model1.indices])
  MSE.nn_VisOnly_train2[[modelCounter]] <- sum(VisOnly_train_scaled$weightsG0*
                                                (VisOnly_train_scaled$yVisOnlySqrt^2 - 
                                                pr.nn_VisOnly_train[[modelCounter]]^2)^2)/
                                                nrow(VisOnly_train_scaled[model1.indices])
  ...
  MSE.nn_VisOnly_test[[modelCounter]] <- sum(VisOnly_test_scaled$weightsG0*
                                               (VisOnly_test_scaled$yVisOnlySqrt - 
                                                pr.nn_VisOnly_test[[modelCounter]])^2)/
                                                nrow(VisOnly_test_scaled[model1.indices])
  MSE.nn_VisOnly_test2[[modelCounter]] <- sum(VisOnly_test_scaled$weightsG0*
                                               (VisOnly_test_scaled$yVisOnlySqrt^2 - 
                                                pr.nn_VisOnly_test[[modelCounter]]^2)^2)/
                                                nrow(VisOnly_test_scaled[model1.indices])
  ...
  MSE.nn_Joint_train[[modelCounter]]  <- sum(Joint_train_scaled$weightsG0*(Joint_train_scaled$ySqrt - 
                                                pr.nn_Joint_train[[modelCounter]])^2)/
                                                nrow(Joint_train_scaled[model1.indices])
  MSE.nn_Joint_train2[[modelCounter]]  <- sum(Joint_train_scaled$weightsG0*(Joint_train_scaled$ySqrt^2 - 
                                                pr.nn_Joint_train[[modelCounter]]^2)^2)/
                                                nrow(Joint_train_scaled[model1.indices])
    
  pr.nn_Joint_test[[modelCounter]]  <- predict(nn_Joint[[modelCounter]],Joint_test_scaled[,model1.indices],na.action=na.omit)
  MSE.nn_Joint_test[[modelCounter]]  <- sum(Joint_test_scaled$weightsG0*(Joint_test_scaled$ySqrt - 
                                               pr.nn_Joint_test[[modelCounter]])^2)/
                                               nrow(Joint_test_scaled[model1.indices])
  
  MSE.nn_Joint_test2[[modelCounter]]  <- sum(Joint_test_scaled$weightsG0*(Joint_test_scaled$ySqrt^2 - 
                                               pr.nn_Joint_test[[modelCounter]]^2)^2)/
                                               nrow(Joint_test_scaled[model1.indices])
...
load(paste(outDir,SP,'_AcOnly_NN_density.Rdata',sep=''))
load(paste(outDir,SP,'_VisOnly_NN_density.Rdata',sep=''))
load(paste(outDir,SP,'_Joint_NN_density.Rdata',sep=''))
```
...
for (iM in 1: length(nn_AcOnly)){
  XEntropy.AcOnly_train[[iM]] <- LogLoss(pmax(pr.nn_AcOnly_train[[iM]],0),
                                         AcOnly_train_scaled$yAcOnlySqrt)
  XEntropy.AcOnly_test[[iM]] <- LogLoss(pmax(pr.nn_AcOnly_test[[iM]],0),
                                        AcOnly_test_scaled$yAcOnlySqrt)
  XEntropy.VisOnly_train[[iM]] <- weighted_logloss(pmax(pr.nn_VisOnly_train[[iM]],0),
                                         VisOnly_train_scaled$yVisOnlySqrt,
                                         VisOnly_train_scaled$weightsG0)
  XEntropy.VisOnly_test[[iM]] <- weighted_logloss(pmax(pr.nn_VisOnly_test[[iM]],0),
                                         VisOnly_test_scaled$yVisOnlySqrt,
                                         VisOnly_test_scaled$weightsG0)
  XEntropy.Joint_train[[iM]] <- weighted_logloss(pr.nn_Joint_train[[iM]],
                                         Joint_train_scaled$ySqrt,
                                         Joint_train_scaled$weightsG0)
  XEntropy.Joint_test[[iM]] <- weighted_logloss(pr.nn_Joint_test[[iM]],
                                         Joint_test_scaled$ySqrt,
...
for (iM in 1: length(nn_AcOnly)){
  KSStat.AcOnly_train[[iM]] <- KS_Stat(pr.nn_AcOnly_train[[iM]],AcOnly_train_scaled$yAcOnly_TF)
  KSStat.AcOnly_test[[iM]] <- KS_Stat(pr.nn_AcOnly_test[[iM]],AcOnly_test_scaled$yAcOnly_TF)
  KSStat.VisOnly_train[[iM]] <- KS_Stat(pr.nn_VisOnly_train[[iM]],VisOnly_train_scaled$yVisOnly_TF)
  KSStat.VisOnly_test[[iM]] <- KS_Stat(pr.nn_VisOnly_test[[iM]],VisOnly_test_scaled$yVisOnly_TF)
  KSStat.Joint_train[[iM]] <- KS_Stat(pr.nn_Joint_train[[iM]],Joint_train_scaled$y_TF)
  KSStat.Joint_test[[iM]] <- KS_Stat(pr.nn_Joint_test[[iM]],Joint_test_scaled$y_TF)
  
  
  
  MSE.nn_AcOnly_test2[[iM]]  <- sum((AcOnly_test_scaled$yAcOnlySqrt^2 - 
                                               pmax(pr.nn_AcOnly_test[[iM]],0)^2)^2)/
                                               nrow(AcOnly_test_scaled[model1.indices])
  MSE.nn_VisOnly_test2[[iM]]  <- sum(VisOnly_test_scaled$weightsG0*
                                       (VisOnly_test_scaled$yVisOnlySqrt^2 - 
                                               pmax(pr.nn_VisOnly_test[[iM]],0)^2)^2)/
                                               nrow(VisOnly_test_scaled[model1.indices])
  MSE.nn_Joint_test2[[iM]]  <- sum(Joint_test_scaled$weightsG0*
                                             (Joint_test_scaled$ySqrt^2 - 
                                               pmax(pr.nn_Joint_test[[iM]],0)^2)^2)/
                                               nrow(Joint_test_scaled[model1.indices])
}
...
MSE.nn_AcOnly_test2[[iM]]  <- sum((AcOnly_test_scaled$yAcOnlySqrt^2 - 
                                               pmax(pr.nn_AcOnly_test[[iM]],0)^2)^2)/
                                               nrow(AcOnly_test_scaled[model1.indices])
MSE.nn_Joint_test2[[modelCounter]]  <- sum(Joint_test_scaled$weightsG0*
                                             (Joint_test_scaled$ySqrt^2 - 
                                               pmax(pr.nn_Joint_test[[modelCounter]],0)^2)^2)/
                                               nrow(Joint_test_scaled[model1.indices])
...
Note: There may be a challenge here because Stenella are so common at the acoustic sites that there isn't much of a relationsip to anything when you look at things in terms of presence/absence.
...
plot.nnet(nn_Joint[[best_Joint_ModelIndex]]$model[[1]],x.lab = n2, y.lab = "Density",bias = F,cex.val=2)
dev.off()
```
***Example network:***

![](E:/NASData/ModelData/Ssp/Ssp_exampleNetwork.png)
...
predAcOnly_DC <- predict(nn_AcOnly[[best_AcOnly_ModelIndex]],AcOnly_test_scaled[compAcSet_DC,model1.indices])
predAcOnly_MP <- predict(nn_AcOnly[[best_AcOnly_ModelIndex]],AcOnly_test_scaled[compAcSet_MP,model1.indices])
...
AcOnly_predictionSet$predSd_MC[m1] <- rowSds(data.matrix(predVar_MC_DF),na.rm=TRUE)
...
AcOnly_predictionSet$predSd_GC[m2] <- rowSds(data.matrix(predVar_GC_DF))
...
AcOnly_predictionSet$predSd_DT[m3] <- rowSds(data.matrix(predVar_DT_DF))
...
AcOnly_predictionSet$predSd_DC[m4] <- rowSds(data.matrix(predVar_DC_DF),na.rm=TRUE)
...
AcOnly_predictionSet$predSd_MP[m5] <- rowSds(data.matrix(predVar_MP_DF),na.rm=TRUE)


AcOnly_predictionSet$Legend <- "Predictions"
AcOnly_predictionSet$StDev <- "Std. Dev."
AcOnly_test_scaled$Legend <- "Observations"

```

**Predicted and observed encounter probabilities at passive acoustic sites using the acoustic-only model (Site order: MC, GC, DT, DC, MP):**
...
p1_Ac <- ggplot() +
  ...
  geom_line(data = AcOnly_test_scaled[compAcSet_MC,],
             aes(x = date, y = yAcOnly_TF, color = Legend)) +
  labs(y = "", x = "")+
...
p2_Ac <- ggplot() +
  ...
  geom_line(data = AcOnly_test_scaled[compAcSet_GC,],
             aes(x =date, y = yAcOnly_TF, color = Legend))+
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "", x = "")+theme_bw()+
...
p3_Ac <- ggplot() +
  ...
  geom_line(data = AcOnly_test_scaled[compAcSet_DT,],
             aes(x =as.POSIXct(date), y = yAcOnly_TF, color = Legend))+
  geom_line(data = AcOnly_predictionSet,
...
p4_Ac <- ggplot() +
  ...
  geom_line(data = AcOnly_test_scaled[compAcSet_DC,],
             aes(x =as.POSIXct(date), y = yAcOnly_TF, color = Legend))+  
  geom_line(data = AcOnly_predictionSet,
...
p5_Ac <- ggplot() +
  ...
  geom_line(data = AcOnly_test_scaled[compAcSet_MP,],
             aes(x =as.POSIXct(date), y = yAcOnly_TF, color = Legend))+
  geom_line(data = AcOnly_predictionSet,
...
VisOnly_predictionSet$predSd_DC[m4] <- rowSds(data.matrix(predVar_DC_DF))
...
VisOnly_predictionSet$predSd_MP[m5] <- rowSds(data.matrix(predVar_MP_DF))
...
pred_DC <- predict(nn_Joint[[best_Joint_ModelIndex]],AcOnly_test_scaled[compAcSet_DC,model1.indices])
pred_MP <- predict(nn_Joint[[best_Joint_ModelIndex]],AcOnly_test_scaled[compAcSet_MP,model1.indices])
...
Joint_predictionSet$predSd_DC[m4] <- rowSds(data.matrix(predVar_DC_DF))
...
Joint_predictionSet$predSd_MP[m5] <- rowSds(data.matrix(predVar_MP_DF))
...
for (iMod in c(1:trainRepeats)){
  perModel_jan2009_AcOnly[iMod] <- raster::predict(jan2009_rasters,
                                             nn_AcOnly[[best_AcOnly_ModelIndex]]$model[[iMod]],
                                             na.action = na.pass)
  perModel_july2009_AcOnly[iMod] <- raster::predict(july2009_rasters,
                                             nn_AcOnly[[best_AcOnly_ModelIndex]]$model[[iMod]],
                                             na.action = na.pass)
}
```
...
for (iMod in c(1:trainRepeats)){
  perModel_jan2009_VisOnly[iMod] <- raster::predict(jan2009_rasters,
                                             nn_VisOnly[[best_VisOnly_ModelIndex]]$model[[iMod]],
                                             na.action = na.pass)
  perModel_july2009_VisOnly[iMod] <- raster::predict(july2009_rasters,
                                             nn_VisOnly[[best_VisOnly_ModelIndex]]$model[[iMod]],
                                             na.action = na.pass)
}
```
...
for (iMod in c(1:trainRepeats)){
  perModel_jan2009_Joint[iMod] <- raster::predict(jan2009_rasters,
                                             nn_Joint[[best_Joint_ModelIndex]]$model[[iMod]],
                                             na.action = na.pass)
  perModel_july2009_Joint[iMod] <- raster::predict(july2009_rasters,
                                             nn_Joint[[best_Joint_ModelIndex]]$model[[iMod]],
                                             na.action = na.pass)
}
```
...
perModel_jan2009_AcOnly_proj <- vector('list',length = trainRepeats)
perModel_jan2009_AcOnly_crop <- vector('list',length = trainRepeats)
perModel_july2009_AcOnly_proj <- vector('list',length = trainRepeats)
perModel_july2009_AcOnly_crop <- vector('list',length = trainRepeats)
for (iMod in c(1:trainRepeats)){
  perModel_jan2009_AcOnly_proj[[iMod]] <- projectRaster(perModel_jan2009_AcOnly[[iMod]], crs=map_proj)
  perModel_jan2009_AcOnly_crop[[iMod]]  <- mask(perModel_jan2009_AcOnly_proj[[iMod]],predict_template_proj)

  perModel_july2009_AcOnly_proj[[iMod]] <- projectRaster(perModel_july2009_AcOnly[[iMod]], crs=map_proj)
  perModel_july2009_AcOnly_crop[[iMod]]  <- mask(perModel_july2009_AcOnly_proj[[iMod]],predict_template_proj)
}
predCV_jan2009_AcOnly_crop <- calc(stack(perModel_jan2009_AcOnly_crop),fun=cv)/100
predCV_july2009_AcOnly_crop <- calc(stack(perModel_july2009_AcOnly_crop),fun=cv)/100
...
perModel_jan2009_VisOnly_proj <- vector('list',length = trainRepeats)
perModel_jan2009_VisOnly_crop <- vector('list',length = trainRepeats)
perModel_july2009_VisOnly_proj <- vector('list',length = trainRepeats)
perModel_july2009_VisOnly_crop <- vector('list',length = trainRepeats)
for (iMod in c(1:trainRepeats)){
  perModel_jan2009_VisOnly_proj[[iMod]] <- projectRaster(perModel_jan2009_VisOnly[[iMod]], crs=map_proj)
  perModel_jan2009_VisOnly_crop[[iMod]]  <- mask(perModel_jan2009_VisOnly_proj[[iMod]], predict_template_proj)
  perModel_july2009_VisOnly_proj[[iMod]] <- projectRaster(perModel_july2009_VisOnly[[iMod]], crs=map_proj)
  perModel_july2009_VisOnly_crop[[iMod]]  <- mask(perModel_july2009_VisOnly_proj[[iMod]], predict_template_proj)
}
predCV_jan2009_VisOnly_crop <- calc(stack(perModel_jan2009_VisOnly_crop),fun=cv)/100
predCV_july2009_VisOnly_crop <- calc(stack(perModel_july2009_VisOnly_crop),fun=cv)/100
...
perModel_jan2009_Joint_proj <- vector('list',length = trainRepeats)
perModel_jan2009_Joint_crop <- vector('list',length = trainRepeats)
perModel_july2009_Joint_proj <- vector('list',length = trainRepeats)
perModel_july2009_Joint_crop <- vector('list',length = trainRepeats)
for (iMod in c(1:trainRepeats)){
  perModel_jan2009_Joint_proj[[iMod]] <- projectRaster(perModel_jan2009_Joint[[iMod]], crs=map_proj)
  perModel_jan2009_Joint_crop[[iMod]]  <- mask(perModel_jan2009_Joint_proj[[iMod]], predict_template_proj)

  perModel_july2009_Joint_proj[[iMod]] <- projectRaster(perModel_july2009_Joint[[iMod]], crs=map_proj)
  perModel_july2009_Joint_crop[[iMod]]  <- mask(perModel_july2009_Joint_proj[[iMod]], predict_template_proj)
}
predCV_jan2009_Joint_crop <- calc(stack(perModel_jan2009_Joint_crop),fun=cv)/100
predCV_july2009_Joint_crop <- calc(stack(perModel_july2009_Joint_crop),fun=cv)/100
```
<br>
...
```{r leaflet summer density, message = FALSE, warning = FALSE, echo = FALSE}
...
```{r leaflet summer density uncertainty, message = FALSE, warning = FALSE, echo = FALSE}
# Display summer (July 2009) map:
maxColor <- ceiling(max(c(predCV_july2009_AcOnly_crop@data@max,
                predCV_july2009_VisOnly_crop@data@max,
                predCV_july2009_Joint_crop@data@max))*10)/10
pal <- colorNumeric(palette = matlab.like2(5),
                    domain=c(0,maxColor), 
                    na.color = 'transparent')
GU2009Effort = shapefile('E:/NASData/GU2009Effort/GU_Effort_Merge_clip.shp')
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  addProviderTiles(providers$Esri.OceanBasemap) %>%
  addRasterImage(predCV_july2009_AcOnly_crop, colors = pal,
                 opacity = 0.8, group = 'Acoustic July 2009') %>%
  addRasterImage(predCV_july2009_VisOnly_crop, colors = pal,
                 opacity = 0.8, group = 'Visual July 2009') %>%
  addRasterImage(predCV_july2009_Joint_crop, colors = pal,
                 opacity = 0.8, group = 'Joint July 2009') %>%
  addLegend(pal = pal, values = c(0,maxColor),
    title = 'CV(P(encounter))',position = "bottomleft") %>%
  addLayersControl(
    baseGroups = c('Acoustic July 2009','Visual July 2009',
                   'Joint July 2009'),
    options = layersControlOptions(collapsed = FALSE)
...
```{r leaflet winter density, message = FALSE, warning = FALSE, echo = FALSE}
...
```{r leaflet winter uncertainty, message = FALSE, warning = FALSE, echo = FALSE}
# Display summer (July 2009) map:
maxColor <- ceiling(max(c(predCV_jan2009_AcOnly_crop@data@max,
                predCV_jan2009_VisOnly_crop@data@max,
                predCV_jan2009_Joint_crop@data@max))*10)/10
pal <- colorNumeric(palette = matlab.like2(5),
                    domain=c(0,maxColor), 
                    na.color = 'transparent')
GU2009Effort = shapefile('E:/NASData/GU2009Effort/GU_Effort_Merge_clip.shp')
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  addProviderTiles(providers$Esri.OceanBasemap) %>%
  addRasterImage(predCV_jan2009_AcOnly_crop, colors = pal,
                 opacity = 0.8, group = 'Acoustic Jan. 2009') %>%
  addRasterImage(predCV_jan2009_VisOnly_crop, colors = pal,
                 opacity = 0.8, group = 'Visual Jan. 2009') %>%
  addRasterImage(predCV_jan2009_Joint_crop, colors = pal,
                 opacity = 0.8, group = 'Joint Jan. 2009') %>%
  addLegend(pal = pal, values = c(0,maxColor),
    title = 'CV(P(encounter))',position = "bottomleft") %>%
  addLayersControl(
...
for (iM in 1:length(monthStr)){
  climatePrediction[[iM]] <- raster::predict(raster_set[[iM]],nn_AcOnly[[best_Joint_ModelIndex]],
           na.action = na.pass)
