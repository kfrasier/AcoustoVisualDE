...
#fac3 <- array(data = NA, dim = c(length(transformedCovars.train$fac1),1)) # fac 3 has visuals as one group, acoustics as another, only 2 categories.
fac4 <- array(data = NA, dim = c(length(transformedCovars.train$fac1),1)) # fac4 keeps all visuals as one group, groups acoustics by site.
# Add down weighting for visual zeros based on estimate of G0.
...
for (iFac in 1:length(transformedCovars_VisOnly.train$fac1)) {
  if (Train_VisOnly.set$Density[iFac]==0){
    # if it's visual data and it's a zero, adjust by g0 ie, only an X% chance it was a true zero.
    weightsG0[iFac,1] <- visG0
  }
}
...
gamm_full_AcOnly_TF$v01 <- gamm(yAcOnly_TF ~  s(SST, bs='ts', k=kVal)
                               ...
                               #random = ~(1|fac1))# gamm4 format
                               #random= list(fac1=~1)# gamm version
...
gamm_full_VisOnly_TF$v01 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(log10_HYCOM_MAG_0,bs='ts', k=kVal)
                               + s(EddyDist, bs='ts', k=kVal)
...
gamm_full_TF$v01 <- gamm(y_TF~  s(SST, bs='ts', k=kVal)
                        ...
                        na.action = na.omit,family = quasibinomial(),
                        correlation = corAR1(form=~1|fac2),niterPQL = 20)
```
...
gamm_full_TF$v02 <- gamm(y_TF~ s(SST, bs='ts', k=kVal)
                        ...
                        weights = transformedCovars.train$weightsG0*
                          (mergedTrain.set$Weight/max(mergedTrain.set$Weight)),
                        na.action = na.omit,family = quasibinomial(),
                        correlation = corAR1(form=~1|fac4),niterPQL = 20)
```
...
gamm_full_AcOnly_TF$v03 <- gamm(yAcOnly_TF ~ s(SST, bs='ts', k=kVal)
                               ...
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal),
                               data = transformedCovars_AcOnly.train,
                               control = lmeControl(opt= "optim",niterEM = 0, msMaxIter = 100),
                               na.action = na.omit,family = quasibinomial(),
                               correlation = corAR1(form=~1|fac2))#
```
...
gamm_full_VisOnly_TF$v03 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(log10_HYCOM_MAG_0,bs='ts', k=kVal)
...
gamm_full_VisOnly_TF$v04 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH,EddyDist, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(log10_HYCOM_MAG_0,bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
...
for (iMod in 1:length(gamm_full_VisOnly_TF)){
  Vis_model_AIC[iMod] <- (gamm_full_VisOnly_TF[[iMod]]$gcv.ubre)
...
VisOnly_gamm_pruned_TF_best <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(log10_HYCOM_MAG_0,bs='ts', k=kVal)
                               + s(Pos_EddyDist, bs='ts', k=kVal)
...
summary(VisOnly_gamm_pruned_TF_best)
plot(VisOnly_gamm_pruned_TF_best,pages=1,cex.lab = 1.3,cex.axis = 1.1,scale=0)
save(VisOnly_gamm_pruned_TF_best,file = paste(outDir,SP,'_best_VisOnly_binomial_model.Rdata',sep=''))

```

```{r Vis best model residuals}
plot(residuals.gam(VisOnly_gamm_pruned_TF_best))
...
gamm_pruned_TF_best <- gamm(y_TF~ s(SSH, bs='ts', k=kVal) 
                        + s(HYCOM_SALIN_0, bs='ts', k=kVal),
                        data = transformedCovars.train, 
                        weights =  transformedCovars.train$weightsG0*(mergedTrain.set$Weight/max(mergedTrain.set$Weight)),
                        na.action = na.omit,family = quasibinomial(),
...
predVisOnly_MC <- predict.gam(VisOnly_gamm_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_MC,],
                              type = 'response',na.action = na.pass)
predVisOnly_GC <- predict.gam(VisOnly_gamm_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_GC,],
                              type = 'response',na.action = na.pass)
predVisOnly_DT <- predict.gam(VisOnly_gamm_pruned_TF_best,
                              transformedCovars_AcOnly.test[compAcSet_DT,],
                              type = 'response',na.action = na.pass)
...
jan2009_VisOnly_prediction <- raster::predict(jan2009_rasters,VisOnly_gamm_pruned_TF_best,
            type = 'response',na.action = na.pass)
            
july2009_VisOnly_prediction <- raster::predict(july2009_rasters,VisOnly_gamm_pruned_TF_best,
            type = 'response',na.action = na.pass)    
```
