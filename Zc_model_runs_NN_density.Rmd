...
p2 <- ggplot() +
  geom_rect(data =VisOnly_predictionSet, aes(xmin=date[731], xmax=date[1095], ymin=0, ymax=10), fill="gray85")+
  geom_col(data = AcOnly_predictionSet,
  ...
  scale_color_manual(values= c("grey36","green3","deepskyblue"))+theme_bw()+
  theme(legend.position="none", axis.title.y = element_text(size = 8,),panel.spacing.y=unit(0, "lines"),plot.margin = unit(c(0,0,10,0), "pt"),axis.text.x = element_blank())+
  labs(y =bquote("Density (per 1000"~km^2~")"), x = NULL)+
  annotation_custom(gcLabel)
...
p3 <- ggplot() +
  ...
  #          aes(x = date, y = pmax(DT - predSd_DT,0)), fill = "white") +
  geom_rect(data =VisOnly_predictionSet, aes(xmin=date[731], xmax=date[1095], ymin=0, ymax=30), fill="gray85")+
  geom_col(data = AcOnly_predictionSet,
  ...
  labs(y =" ", x = NULL)+theme_bw()+coord_cartesian(ylim = c(0,30))+
  theme(legend.position="none",axis.title.y = element_text(size = 8,),panel.spacing.y=unit(0, "lines"),plot.margin = unit(c(0,0,0,12), "pt"))+
  annotation_custom(dtLabel)
