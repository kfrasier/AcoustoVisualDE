...
RMSE$AcOnly_test_JointData <- sqrt(mean(as.vector(joint_test_weightsG0)*
                                    ((as.vector(transformedCovars.test$y)-
                                    pr$AcOnly_JointData$fit)^2),na.rm = TRUE))
RMSE$VisOnly_test_JointData <- sqrt(mean(as.vector(joint_test_weightsG0)*
                                    ((as.vector(transformedCovars.test$y)-
                                    pr$VisOnly_JointData$fit)^2),na.rm = TRUE))
RMSE$Joint_test <- sqrt(mean(as.vector(joint_test_weightsG0)*
                                    ((as.vector(transformedCovars.test$y)-
                                    pr$Joint_JointData$fit)^2),na.rm = TRUE))



RMSETable <- rbind(RMSE$AcOnly_test_JointData,RMSE$VisOnly_test_JointData,
                      RMSE$Joint_test)


MAETable<- rbind(MAE$AcOnly_test_JointData,MAE$VisOnly_test_JointData,
                      MAE$Joint_test)

colnames(RMSETable)<- 'RMSE'#'Mean Absolute Error'
