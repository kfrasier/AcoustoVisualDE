...
MSE$AcOnly_test_JointData <- mean(as.vector(joint_test_weightsG0)*
                                    abs(as.vector(transformedCovars.test$y)-
                                    pr$AcOnly_JointData$fit),na.rm = TRUE)
MSE$VisOnly_test_JointData <- mean(as.vector(joint_test_weightsG0)*
                                    abs(as.vector(transformedCovars.test$y)-
                                    pr$VisOnly_JointData$fit),na.rm = TRUE)
MSE$Joint_test <- mean(as.vector(joint_test_weightsG0)*
                         abs(as.vector(transformedCovars.test$y)-
...
AcMapSummer = tm_shape(july2009_AcOnly_prediction_crop)+
  tm_raster(palette = viridis(9, begin = 0,end = max(july2009_AcOnly_prediction_crop@data@values,
            na.rm = TRUE)/maxColor),colorNA = NULL,style = "cont",
            title = expression("Animals/100"~km^{2}),legend.reverse = TRUE)+ 
