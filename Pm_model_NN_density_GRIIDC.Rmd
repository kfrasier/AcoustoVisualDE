...
outDir <- file.path('F:/GRIIDC_inputs/ModelData',SP,'modelOutput//')
...
load("F:/GRIIDC_inputs/JointScalingFactors.Rdata")
# Scale training data for the NN
covars_AcOnly_max.train <- apply(AcOnly.train.NoNa, 2, max, na.rm = TRUE) 
covars_Joint_min.train <- apply(AcOnly.train.NoNa, 2, min, na.rm = TRUE)
...
covars_Joint_min.train <- apply(VisOnly.train.NoNa, 2, min, na.rm = TRUE)
...
AcOnly_train_scaled$EffectiveArea<-0
VisOnly_train_scaled$EffectiveArea<-0
...
for (layerSize in layerSizeList){
  ...
  MSE$nn_AcOnly_train[[AcCounter]] <- mean(abs(exp(log(covars_Joint_max.train['y'])*
				  		(AcOnly_train_scaled$yAcOnly - 
...
p2 <- ggplot() +
  ...
  theme(legend.position="none", axis.title.y = element_text(size = 8,),panel.spacing.y=unit(0, "lines"),
        plot.margin = unit(c(0,0,10,9), "pt"),axis.text.x = element_blank())+
