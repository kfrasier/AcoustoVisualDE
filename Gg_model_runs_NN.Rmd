...
AcOnly_predictionSet$predSd_DC[m4] <- rowSds(data.matrix(predVar_DC_DF))/trainRepeats
...
AcOnly_predictionSet$predSd_MP[m5] <- rowSds(data.matrix(predVar_MP_DF))/trainRepeats
...
**Predicted and observed occurrence at passive acoustic monitoring sites using the acoustic-only model:**
...
```{r look at effect of hidden layer on spatial prediction}
nModels = 6;
...
# Display winter (January 2009) map:
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 5)%>%
  addProviderTiles(providers$Esri.OceanBasemap) %>%
