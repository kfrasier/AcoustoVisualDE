...
source('F:/NASData/AcoustoVisualDE/AcoustoVisualDE/HabitatProject/R/weighted_logloss.R')
...
keepDates_AcOnly.train <- which(yearListIdx_AcOnly != 2009 & 
                  yearListIdx_AcOnly >= 2003 & yearListIdx_AcOnly <= 2014)

keepDates_AcOnly.test <- which(yearListIdx_AcOnly == 2016 |
                                 yearListIdx_AcOnly == 2015)
...
outlierList <-which(Train_AcOnly.set$SST <1)
Train_AcOnly.set$SST[outlierList] <- NaN 
...
outlierList <-which(Test_AcOnly.set$SST< 1)
Test_AcOnly.set$SST[outlierList] <- NaN 
...
```{r load models, eval = FALSE, echo = FALSE}
...
RMSEtable <- rbind(RMSE$nn_AcOnly_test_allData,RMSE$nn_VisOnly_test_allDat)
wRMSEtable <- rbind(wRMSE$nn_AcOnly_test_allData,wRMSE$nn_VisOnly_test_allDat)

MAEtable <- rbind(MAE$nn_AcOnly_test_allData,MAE$nn_VisOnly_test_allDat)
wMAEtable <- rbind(wMAE$nn_AcOnly_test_allData,wMAE$nn_VisOnly_test_allDat)

colnames(RMSEtable)<- layerSizeList
rownames(RMSEtable)<- c("Acoustic - Joint test data","Visual - Joint test data")
colnames(wRMSEtable)<- layerSizeList
rownames(wRMSEtable)<- c("Acoustic - Joint test data","Visual - Joint test data")
colnames(MAEtable)<- layerSizeList
rownames(MAEtable)<- c("Acoustic - Joint test data","Visual - Joint test data")
colnames(wMAEtable)<- layerSizeList
rownames(wMAEtable)<- c("Acoustic - Joint test data","Visual - Joint test data")

print('RMSE (lower is better)')
print(RMSEtable, digits = 3)

print('wRMSE (lower is better)')
print(wRMSEtable, digits = 4)
...
# cv_var_importance$Joint <- Joint_node_cv
...
dev.off()
egNetFig <-paste0(outDir,SP,'_exampleNetwork.tiff')
```
***Example network:***
...
predAcOnly_MC <- exp(unname(predict(nn_AcOnly[[best_AcOnly_ModelIndex]],
                         reducedData_MC))*log(covars_Joint_max.train['y']))-1
...
predAcOnly_GC <- exp(unname(predict(nn_AcOnly[[best_AcOnly_ModelIndex]],
                         reducedData_GC))*log(covars_Joint_max.train['y']))-1
...
predAcOnly_DT <- exp(unname(predict(nn_AcOnly[[best_AcOnly_ModelIndex]],
                         reducedData_DT))*log(covars_Joint_max.train['y']))-1
...
predAcOnly_DC <- exp(unname(predict(nn_AcOnly[[best_AcOnly_ModelIndex]],
                         reducedData_DC))*log(covars_Joint_max.train['y']))-1
...
predAcOnly_MP <- exp(unname(predict(nn_AcOnly[[best_AcOnly_ModelIndex]],
                         reducedData_MP))*log(covars_Joint_max.train['y']))-1
...
p1_Ac <- ggplot() +
  ...
  theme(legend.position="right",axis.title.y = element_text(size = 8,))+
...
png(paste(outDir,SP,'_AcOnly_Timeseries_Prediction_NN_Density.png',sep=''), width = 6.5, height = 4, units = "in", res=300)
...
predVisOnly_MC <- exp(predict(nn_VisOnly[[best_VisOnly_ModelIndex]],
                          reducedData_MC)*
                          log(covars_Joint_max.train['y']))-1
...
# pred_MC <- exp(predict(nn_Joint[[best_Joint_ModelIndex]],
#                    reducedData_MC)*log(covars_Joint_max.train['y']))-1
# pred_MC[pred_MC<0]<-0
# pred_GC <- exp(predict(nn_Joint[[best_Joint_ModelIndex]],
#                    reducedData_GC)*log(covars_Joint_max.train['y']))-1
# pred_GC[pred_GC<0]<-0
# 
# pred_DT <- exp(predict(nn_Joint[[best_Joint_ModelIndex]],
#                    reducedData_DT)*log(covars_Joint_max.train['y']))-1
# pred_DT[pred_DT<0]<-0
# 
# pred_DC <- exp(predict(nn_Joint[[best_Joint_ModelIndex]],
#                    reducedData_DC)*log(covars_Joint_max.train['y']))-1
# pred_DC[pred_DC<0]<-0
# 
# pred_MP <- exp(predict(nn_Joint[[best_Joint_ModelIndex]],
#                   reducedData_MP)*log(covars_Joint_max.train['y']))-1
# pred_MP[pred_MP<0]<-0
# 
# predVar_MC <- NULL
...
# Joint_predictionSet$predSd_DC[m4] <- rowSds(data.matrix(predVar_DC_DF))#/sqrt(trainRepeats)
# 
...
# Joint_predictionSet$predSd_MP[m5] <- rowSds(data.matrix(predVar_MP_DF))#/sqrt(trainRepeats)
# 
# Joint_predictionSet$Legend <- "Predictions - Train"
# Joint_predictionSet$LegendTest <- "5 Joint"
...
# par(mfrow = c(5,1),oma=c(3,0,5,0)),colour="#000099",colour="#CC0000"
# p1 <- ggplot() +
#   geom_rect(data =VisOnly_predictionSet, aes(xmin=date[731], xmax=date[1095], ymin=0, ymax=45), fill="gray85")+
#   geom_col(data = AcOnly_predictionSet,
...
#   scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual","Model - Joint"),
#                      values= c("grey46","gray34","green3","deepskyblue","red2"),
#                      guide = guide_legend(keyheight = unit(1, units = "mm") , keywidth = unit(2, units = "mm")))+
#   annotation_custom(mcLabel)
# 
# p2 <- ggplot() +
#   geom_rect(data =VisOnly_predictionSet, aes(xmin=date[731], xmax=date[1095], ymin=0, ymax=20), fill="gray85")+
#   geom_col(data = AcOnly_predictionSet,
...
#             aes(x=date, y=GC, color = LegendTest), size = .5) +
  ...
  #           aes(x=date, y=GC),color = "red2", size = .5) +
#   geom_path(data = VisOnly_predictionSet,
#             aes(x=date, y=GC, color = LegendTest),size = .5) +
#   labs(y = "", x = NULL)+coord_cartesian(ylim = c(0,20))+
#   scale_color_manual(labels = c("Observations","Obs. (7-day mean)","Model - Acoustic","Model - Visual","Model - Joint"),
#                      values= c("grey46","gray34","green3","deepskyblue","red2"))+theme_bw()+
...
# p3 <- ggplot() +
  ...
  # geom_path(data = Joint_predictionSet,
  #           aes(x=date, y=DT),color = "red2", size = .5) +
#   geom_path(data = VisOnly_predictionSet,
#             aes(x=date, y=DT, color = LegendTest), size = .5) +
#   scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual","Model - Joint"),
#                      values= c("grey46","gray34","green3","deepskyblue","red2"))+theme_bw()+
...
# 
# p4 <- ggplot() +
#   geom_rect(data =VisOnly_predictionSet, aes(xmin=date[731], xmax=date[1095], ymin=0, ymax=40), fill="gray85")+
#   geom_col(data = AcOnly_predictionSet,
#              aes(x = date, y = obs_DC,color = Legend1),alpha = 1, width = .1,size = .25)+
#   geom_line(data = AcOnly_predictionSet,
#             aes(x=date, y=DC, color = Legend), size = .5) +
  # geom_path(data = Joint_predictionSet,
  #           aes(x=date, y=DC),color = "red2", size = .5) +
#   geom_path(data = VisOnly_predictionSet,
#             aes(x=date, y=DC, color = LegendTest),size = .5) +
#   labs(y = " ", x = NULL)+
#   scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual","Model - Joint"),
#                      values= c("grey46","gray34","green3","deepskyblue","red2"))+theme_bw()+
#   theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,10,9),"pt"),axis.text.x = element_blank())+coord_cartesian(ylim = c(0,40))+
#   annotation_custom(dcLabel)
# 
# p5 <- ggplot() +
#   geom_rect(data =VisOnly_predictionSet, aes(xmin=date[731], xmax=date[1095], ymin=0, ymax=35), fill="gray85")+
#   geom_col(data = AcOnly_predictionSet,
#              aes(x = date, y = obs_MP,color = Legend1),alpha = 1, width = .1,size = .25)+
#   geom_line(data = AcOnly_predictionSet,
#             aes(x=date, y=MP, color = Legend), size = .5) +
  # geom_path(data = Joint_predictionSet,
  #           aes(x=date, y=MP),color = "red2",size = .5) +
  # geom_path(data = VisOnly_predictionSet,
  #           aes(x=date, y=MP, color = LegendTest),size = .5) +
  # labs(y = " ", x = NULL)+
  # scale_color_manual(labels = c("Observations","Obs. (7-day smooth)","Model - Acoustic","Model - Visual","Model - Joint"),
  #                    values= c("grey46","gray34","green3","deepskyblue","red2"))+theme_bw()+
  # theme(legend.position="none",axis.title.y = element_text(size = 8,),plot.margin = unit(c(0,0,0,9),"pt"))+coord_cartesian(ylim = c(0,35))+
  # annotation_custom(mpLabel)



# png(paste(outDir,SP,'_Joint_Timeseries_Prediction_NN_Density.png',sep=''), width = 6.6, height = 6.7, units = "in", res=600)
# 
...
load('F:/NASData/AcoustoVisualDE/AcoustoVisualDE/2009_prediction_rasters_scaled.Rdata') 
```
...
jan2009_AcOnly_prediction <- (exp(raster::predict(jan2009_rastersAc,
                   ...
                   na.action = na.pass)*log(covars_Joint_max.train['y']))-1)/10
...
jan2009_VisOnly_prediction <- (exp(raster::predict(jan2009_rastersVis,
         ...
         log(covars_Joint_max.train['y']))-1)/10
...
# }
# 
# jan2009_prediction <- (exp(raster::predict(jan2009_rastersJoint,
#          nn_Joint[[best_Joint_ModelIndex]],na.action = na.pass)*log(covars_Joint_max.train['y']))-1)/10
# jan2009_prediction[jan2009_prediction<0]<-0
# 
# july2009_prediction <- (exp(raster::predict(july2009_rastersJoint,
#          nn_Joint[[best_Joint_ModelIndex]],na.action = na.pass)*log(covars_Joint_max.train['y']))-1)/10
...
# for (iMod in c(1:trainRepeats)){
#   perModel_jan2009_Joint[iMod] <- (exp(raster::predict(jan2009_rastersJoint,
#                                              nn_Joint[[best_Joint_ModelIndex]]$model[[iMod]],
#                                              na.action = na.pass)*log(covars_Joint_max.train['y']))-1)/10
#   perModel_jan2009_Joint[[iMod]][perModel_jan2009_Joint[[iMod]]<0]<-0
# 
#   perModel_july2009_Joint[iMod] <- (exp(raster::predict(july2009_rastersJoint,
#                                              nn_Joint[[best_Joint_ModelIndex]]$model[[iMod]],
#                                              na.action = na.pass)*log(covars_Joint_max.train['y']))-1)/10
...
# }
# predSTD_jan2009_Joint_crop <- calc(stack(perModel_jan2009_Joint_crop),fun=std_err)/10
# predSTD_july2009_Joint_crop <- calc(stack(perModel_july2009_Joint_crop),fun=std_err)/10
```
...
# JointMapSummer = tm_shape(july2009_prediction_crop)+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",title = expression("Animals/100"~km^{2}),legend.reverse = TRUE)+ 
...
AcMapSummer = tm_shape(july2009_AcOnly_prediction_crop)+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",title = expression("Animals/100"~km^{2}),legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
...
VisMapSummer = tm_shape(july2009_VisOnly_prediction_crop)+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",title = expression("Animals/100"~km^{2}),legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
...
crop_limits_Reproj = spTransform(crop_limits_proj,crs(GU2009EffortLines))
maxColorSummer = 1.5
JointMap1 = tm_shape(july2009_prediction_hidden[[1]])+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",title = expression("Animals/100"~km^{2}),legend.reverse = TRUE)+ tm_layout(panel.labels="4 Nodes",panel.label.size=.6,legend.position = c(0.76,0.35))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
  tm_shape(sightShape)+tm_symbols(col = "black", border.col = "white",shape=21,size = 0.4, alpha = .75)+
  tm_legend(legend.height = .5,legend.title.size	
 = .9)

JointMap2 = tm_shape(july2009_prediction_hidden[[2]])+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",legend.show = FALSE)+ tm_layout(panel.labels="6 Nodes",panel.label.size=.6)+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
...
JointMap3 = tm_shape(july2009_prediction_hidden[[3]])+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",legend.show = FALSE)+ tm_layout(panel.labels="8 Nodes",panel.label.size=.6,legend.position = c(0.89,0.02))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
...
JointMap4 = tm_shape(july2009_prediction_hidden[[4]])+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",legend.show = FALSE)+ tm_layout(panel.labels="10 Nodes",panel.label.size=.6)+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
...
JointMap5 = tm_shape(july2009_prediction_hidden[[5]])+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",legend.show = FALSE)+ tm_layout(panel.labels="12 Nodes",panel.label.size=.6)+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
...
JointMap6 = tm_shape(july2009_prediction_hidden[[6]])+tm_raster(palette = myColormap,colorNA = NULL,style = "cont",legend.show = FALSE)+ tm_layout(panel.labels="14 Nodes",panel.label.size=.6)+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = 0.2, alpha = 1,shape = 24)+
...
tmap_arrange(JointMap1, JointMap2, JointMap3, JointMap4, JointMap5, JointMap6,ncol = 2, nrow = 3)
...
maxColorSE <- ceiling(max(c(predSTD_july2009_AcOnly_crop@data@max,
               predSTD_july2009_VisOnly_crop@data@max,
               predSTD_july2009_Joint_crop@data@max))*10)/10
# plot best summer map for publication
sightShape = SpatialPoints(sightingsTest[c(2,1)],proj4string =  crs(GU2009EffortLines))
harpShape = SpatialPoints(HARPsites[c(1,8,14),c(2,1)],proj4string =  
                            crs(GU2009EffortLines))
...
JointMapSummer = tm_shape(predSTD_july2009_Joint_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(Density)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+
  tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
...
AcMapSummer = tm_shape(predSTD_july2009_AcOnly_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(Density)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
...
VisMapSummer = tm_shape(predSTD_july2009_VisOnly_crop)+tm_raster(palette = matlab.like2(5),colorNA = NULL,style = "cont",midpoint = maxColorSE/2,title = "SE(Density)",legend.reverse = TRUE)+ 
  tm_legend(legend.title.size = .9,legend.position = c("RIGHT","TOP"))+
  tm_shape(crop_limits_Reproj)+tm_borders(lty = 1)+
  tm_shape(GU2009EffortLines)+tm_lines()+
  tm_shape(harpShape)+tm_symbols(col = "orange", border.col = "black",size = .5, alpha = 1,shape = 24)+
