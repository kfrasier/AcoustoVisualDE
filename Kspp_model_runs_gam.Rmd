...
```{r AC model 1, eval = TRUE, warning = FALSE}
...
```{r other models 1, eval = TRUE, include = TRUE, warning = FALSE} 
...
gam_full_TF$v01 <- gam(y_TF~ s(SST, bs='ts', k=kVal)
                              ...
                              weights = transformedCovars.train$weightsG0*
                                (mergedTrain.set$Weight/max(mergedTrain.set$Weight)),niterPQL = 50)
...
```{r AC model 2, eval = TRUE, warning = FALSE}
...
```{r other models 2, eval = TRUE, include = TRUE, warning = FALSE} 
...
gam_full_VisOnly_TF$v02 <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               ...
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal),
                               data = transformedCovars_VisOnly.train,
                               weights = transformedCovars_VisOnly.train$weightsG0* (Train_VisOnly.set$Weight/max(Train_VisOnly.set$Weight)),
                               na.action = na.omit,family = binomial())
...
gam_full_TF$v02 <- gam(y_TF~ s(SST, bs='ts', k=kVal)
                              ...
                              data = transformedCovars.train, 
                              na.action = na.omit,family = binomial(),
                              weights = transformedCovars.train$weightsG0 *(mergedTrain.set$Weight/max(mergedTrain.set$Weight)),niterPQL = 50)
...
```{r AC model 3, eval = TRUE, warning = FALSE}
...
```{r other models 3, eval = TRUE, include = TRUE, warning = FALSE} 
...
```{r AC model 4, eval = TRUE, warning = FALSE}
...
```{r Other models 4, eval = TRUE, include = TRUE, warning = FALSE}
...
for (iMod in 1:length(gam_full_TF)){
  joint_model_AIC[iMod] <- AIC(gam_full_TF[[iMod]])
  cat('Model ', names(gam_full_TF[iMod]), ' AIC = ', joint_model_AIC[iMod], '\n')
}
```
<br>
...
```{r joint model}
best_model_index <- which.min(joint_model_AIC)
best_joint_model <- gam_full_TF[[best_model_index]]
...
Some summaries of the best models show that some terms were not significant.


```{r Ac unpruned model summary}
summary(best_AcOnly_model)
```
```{r best AC model, message=FALSE, results = 'hide', warning=FALSE}
...
```{r best Vis model, message=FALSE, results = 'hide', warning=FALSE}

VisOnly_gam_pruned_TF_best<- gam(y_TF~ s(SST, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal),
                          data = transformedCovars.train, 
...
```{r best model, echo = TRUE, message=FALSE, results = 'hide', warning=FALSE}
...
for (iM in 1:length(monthStr)){
  ...
  raster_set[[iM]] <- brick(SST_climate,SSH_climate,log10_CHL_climate,eddy_climate,
                            neg_eddy_climate, pos_eddy_climate, log10_HYCOM_MAG_climate,
                         HYCOM_SALIN_climate,log10_HYCOM_MLD_climate) 
  names(raster_set[[iM]])<-c('SST','SSH','log10_CHL','EddyDist',
                             'Neg_EddyDist','Pos_EddyDist','log10_HYCOM_MAG_0',
...
for (iM in 1:length(monthStr)){
  ...
  #ouput raster to kml
  kmlImageFileName = paste0(savePath,'/climatology_predictions/', SP,'_',monthStr[iM],'gam_mean_encounter_probability.kml')
