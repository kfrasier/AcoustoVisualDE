...
for (layerSize in layerSizeList){
  ...
  MSE.nn_VisOnly_train[[modelCounter]] <- mean(VisOnly_train_scaled$weightsG0*
                                        abs(VisOnly_train_scaled$yVisOnly - 
  ...
  MSE.nn_VisOnly_test[[modelCounter]] <- mean(VisOnly_test_scaled$weightsG0*
                                        abs(VisOnly_test_scaled$yVisOnly - 
...
```{r leaflet summer, message = FALSE, warning = FALSE, fig.width = 6, fig.height = 3, echo = FALSE}
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  ...
  addLegend(pal = palUncert, values = c(0,maxColorUncert),
    title = 'STD(Est. Density)',position = "bottomleft") %>%
  addLayersControl(
    baseGroups = c('Acoustic July 2009','Visual July 2009',
                   'Joint July 2009'),
    options = layersControlOptions(collapsed = FALSE)
...
```{r leaflet winter, message = FALSE, warning = FALSE, echo = FALSE, fig.width = 7, fig.height = 5}
...
```{r leaflet winter uncert, message = FALSE, warning = FALSE, echo = FALSE,width = 60, height = 30}
# Display summer (July 2009) map:
...
map <- leaflet(width="100%") %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  addProviderTiles(providers$Esri.OceanBasemap) %>%
  addRasterImage(july2009_prediction_hidden[[1]], colors = palSummer,
                 opacity = 0.8, group = '4 nodes') %>%
  addRasterImage(july2009_prediction_hidden[[2]], colors = palSummer,
                  opacity = 0.8, group = '6 nodes') %>%
  addRasterImage(july2009_prediction_hidden[[3]], colors = palSummer,
                  opacity = 0.8, group = '8 nodes') %>%
  addRasterImage(july2009_prediction_hidden[[4]], colors = palSummer,
                  opacity = 0.8, group = '10 nodes') %>%
  addRasterImage(july2009_prediction_hidden[[5]], colors = palSummer,
                  opacity = 0.8, group = '12 nodes') %>%
  addRasterImage(july2009_prediction_hidden[[6]], colors = palSummer,
