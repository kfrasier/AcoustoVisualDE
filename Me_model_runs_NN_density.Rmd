---
titlF: "GoMx Gervais' beaked whale habitat models: Density with neural nets"
author: 
- namF: Kaitlin Frasier
  affiliation: Scripps Institution of Oceanography, UC San Diego
datF: "`r format(Sys.time(), '%d %B %Y')`"
output: 
  html_document:
    toc: true
    toc_depth: 4
...
load('F:/NASData/ModelData/Me/setup_info_Me.Rdata')
load('F:/NASData/ModelData/Me/MeMergedData.Rdata')
...
NotF: Future work could use monthly climatologies (averages) so that older sightings data could be used. Some dynamic drivers like eddy and front locations would not be able to be considered using that approach.
...
best_AcOnly_ModelIndex <-which.min(MAE$nn_AcOnly_test_allData)
print(paste(c('Best acoustic model has', layerSizeList[best_AcOnly_ModelIndex], 
              'nodes.'),collapse = " "))
best_VisOnly_ModelIndex <-which.min(MAE$nn_VisOnly_test_allData)
print(paste(c('Best visual model has', layerSizeList[best_VisOnly_ModelIndex],
            'nodes.'),collapse = " "))
best_Joint_ModelIndex <-which.min(MAE$nn_Joint_test_allData)
...
```
NotF: As an alternative to training joint models, an average surface was computed across the acoustic and visual-only predictions.
...
load('F:/NASData/AcoustoVisualDE/AcoustoVisualDE/climatology_rasters_scaled.Rdata') 
