...
The timseries below show the acoustic data used for modeling. Data from 2011 and 2012 used for training, and 2013 data is held back for testing.
...
```{r dot plots, eval = FALSE, message = FALSE}
...
```{r correlation plots, eval = FALSE, message = FALSE, warning = FALSE}
# without transform
...
```{r presence absence histograms, eval = FALSE, message = FALSE}
plot.covarDensity(transformedCovars.train,
                  colnames(transformedCovars.train),
                  mergedTrain.set$SpPresent)

plot.covarDensity(transformedCovars_AcOnly.train,
                   colnames(transformedCovars_AcOnly.train),
                  Train_AcOnly.set$Density,paste0('AcousticOnly_',SP))		

plot.covarDensity(transformedCovars_VisOnly.train,
                  colnames(transformedCovars_VisOnly.train),
                  Train_VisOnly.set$Density,paste0('VisualOnly_',SP))
```




![Acoustic kernel densities](E:/NASData/ModelData/Gg/AcousticOnly_Gg_density_pres_abs.png)
...
CAVEAT: There are issues of magnitude differences between the predictions and observations. I think part of the problem is that I'm calculating observed probability of encounter as the fraction of 5-minute bins containing Risso's per week, which is heavily skewed toward 0. 
It would probably be better to do # of days per week, although it will be a coarser estimate. Recalculation in progress. 

Alternatively we could compare to weekly densities, but then the y-axis scales are completely different and it may be an apples to oranges comparison. An earlier version used densities, and seemed to suggest that lagging variables might be appropriate...
...
p1_Ac <- ggplot() +
  ...
  geom_point(data = pOccur[occurIdx,],
...
map <- leaflet() %>%  setView(lng = -88.8, lat = 27.0, zoom = 6)%>%
  ...
  addLayersControl(
    ...
    overlayGroups = 'Test Sightings (Summer 2009)',
