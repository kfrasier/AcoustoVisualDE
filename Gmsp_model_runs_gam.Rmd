...
```{r Acoustic only AIC, echo = FALSE}
AC_model_AIC <- NULL
...
```{r Acoustic only model, echo = FALSE}
AcOnly_model_index <- which.min(AC_model_AIC)
AcOnly_model <- gam_full_AcOnly_TF[[AcOnly_model_index]]
AcOnly_model_name <- names(gam_full_AcOnly_TF[AcOnly_model_index])
```
...
VisOnly_model_index <- which.min(Vis_model_AIC)
VisOnly_model <- gam_full_VisOnly_TF[[VisOnly_model_index]]
VisOnly_model_name <- names(gam_full_VisOnly_TF[VisOnly_model_index])
```
...
Joint_model_index <- which.min(Joint_model_AIC)
Joint_model <- gam_full_Joint_TF[[Joint_model_index]]
Joint_model_name <- names(gam_full_Joint_TF[Joint_model_index])
...
par(mfrow = c(nTerms/3,3),mar = c(4,2,1,1))
for  (i in 1:nTerms) {
...
Each grid cell in the following maps represents a 10x10 km square.
<br>
