...
* Acoustic data have a trucation distance  of 1 km for Gervais' beaked whale (95% of group detections occur within this range), for a total monitoring area of $(1.0 km)^{2} \pi$, 
...
* Visual trucation distance for Gervais' beaked whale is estimated as 2.2 km (see figure below) from these data. ESW is multiplied by transect segment length (usually ~10 km) and doubled to estimate total area swept.


<br>  

\[\text{Acoustic to Visual ratio} = \frac{24 hrs \cdot (1 km)^{2} \pi}{(\text{Segment length}/18.52 km/hr) \cdot 5.4 km \cdot \text{Segment length} \cdot 2} = 75.40/23.76 \approx 3.17:1
\]
...
# 
# 
# nn_VisOnly <- train(f.VisOnly_NN1, data=VisOnly_train_scaled,method = 'avNNet',
#       metric="RMSE",na.action = na.omit, maxit=1000,entropy = TRUE,
#       trControl = fitControl,verboseItr = FALSE,
#       weights = VisOnly_train_scaled$weightsG0,
#       tuneGrid = nnetGrid)
# 
# # predict on train data 
# pr.nn_VisOnly_train <- predict(nn_VisOnly,VisOnly_train_scaled[,model1.indices])
# # predict on test data 
# pr.nn_VisOnly_test <- predict(nn_VisOnly,VisOnly_test_scaled[,model1.indices])
# 
# 
# ## JOINT
# nn_Joint <- train(f.Joint_NN1, data=Joint_train_scaled,method = 'avNNet',
#       metric="RMSE",na.action = na.omit, maxit=1000,
#       trControl = fitControl,verboseItr = FALSE,
#       weights = Joint_train_scaled$weightsG0,
#       tuneGrid = nnetGrid)
# 
# # predict on train data 
# pr.nn_Joint_train <- predict(nn_Joint,Joint_train_scaled[,model1.indices])
# # predict on test data 
# pr.nn_Joint_test <- predict(nn_Joint,Joint_test_scaled[,model1.indices])
...
for (layerSize in layerSizeList){
  ...
  MSE.nn_VisOnly_train[[modelCounter]] <- sum(VisOnly_train_scaled$weightsG0*
                                                ...
                                                round(pr.nn_VisOnly_train[[modelCounter]]))^2)/
  ...
  MSE.nn_VisOnly_test[[modelCounter]] <- sum(VisOnly_test_scaled$weightsG0*
                                               (VisOnly_test_scaled$yVisOnly_TF - 
                                                round(pr.nn_VisOnly_test[[modelCounter]]))^2)/
  ...
  MSE.nn_Joint_train[[modelCounter]]  <- sum(Joint_train_scaled$weightsG0*(Joint_train_scaled$y_TF - 
                                                round(pr.nn_Joint_train[[modelCounter]]))^2)/
  ...
  MSE.nn_Joint_test[[modelCounter]]  <- sum(Joint_test_scaled$weightsG0*(Joint_test_scaled$y_TF - 
                                               round(pr.nn_Joint_test[[modelCounter]] ))^2)/
...
for (iM in 1: length(nn_AcOnly)){
  XEntropy.AcOnly_train[[iM]] <- LogLoss(pr.nn_AcOnly_train[[iM]],AcOnly_train_scaled$yAcOnly_TF)
  XEntropy.AcOnly_test[[iM]] <- LogLoss(pr.nn_AcOnly_test[[iM]],AcOnly_test_scaled$yAcOnly_TF)
  XEntropy.VisOnly_train[[iM]] <- weighted_logloss(pr.nn_VisOnly_train[[iM]],VisOnly_train_scaled$yVisOnly_TF,
                                         VisOnly_train_scaled$weightsG0)
  XEntropy.VisOnly_test[[iM]] <- weighted_logloss(pr.nn_VisOnly_test[[iM]],VisOnly_test_scaled$yVisOnly_TF,
                                         VisOnly_test_scaled$weightsG0)
  XEntropy.Joint_train[[iM]] <- weighted_logloss(pr.nn_Joint_train[[iM]],Joint_train_scaled$y_TF,
                                         Joint_train_scaled$weightsG0)
  XEntropy.Joint_test[[iM]] <- weighted_logloss(pr.nn_Joint_test[[iM]],Joint_test_scaled$y_TF,
...
Calculate mean importance of each input variable in the best models across the 50 iterations.
...
```{r Ac Temporal Predict, echo = FALSE, warning = TRUE}
...
```{r,fig.height = 10, fig.width = 7, message = TRUE, warning = TRUE, echo = FALSE}
...
```{r Vis Temporal Predict, warning = TRUE, echo = FALSE}
...
jan2009_AcOnly_prediction <- raster::predict(jan2009_rasters,nn_AcOnly,
                                             na.action = na.pass)
            
july2009_AcOnly_prediction <- raster::predict(july2009_rasters,nn_AcOnly,na.action = na.pass)    
```
...
jan2009_VisOnly_prediction <- raster::predict(jan2009_rasters,nn_VisOnly,na.action = na.pass)
            
july2009_VisOnly_prediction <- raster::predict(july2009_rasters,nn_VisOnly,
            na.action = na.pass)    
```
...
jan2009_prediction <- raster::predict(jan2009_rasters,nn_Joint,na.action = na.pass)
            
july2009_prediction <- raster::predict(july2009_rasters,nn_Joint,na.action = na.pass)    
```
...
Spatial joint model predictions using oceanographic variables averaged by month between 2003 and 2015.
