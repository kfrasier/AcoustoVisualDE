...
acousticDensityFile <-"/Volumes/EXTHARD/NAS/ALLSITES_binsize000800_Ssp_density_jahStart.csv"# "E:/NASData/GC_DT_binsize000800_Group_density_Zc.csv" # acoustic input file
...
if (matchACSegs){
  ...
  for (iR in 1:nrow(acDensityAll)){
    ...
    if (empty(goodLat)){
      cat("WARNING: No latitude matches found, check decimal places.\n")
    }
...
mergedSegments$Neg_EddyDist <- c(visSeg_OnEffort$Neg_EddyDist,acDensityAll$NEG_EDDYDIST)
...
myLatLon = Train_AcOnly.set[c(3,4)]
...
cat("Run full binomial GAMM on Acoustic only data with shrinkage\n")#random = list(fac1=~1),
...
# vis.gam(gam_full_AcOnly_TF$gam)
sink()

# Predict on test data at MC, for comparison...
compAcSet <- which(ceiling(Test_AcOnly.set$long)==-84)
pred <- predict.gam(gam_full_AcOnly_TF$gam,transformedCovars_AcOnly.test[compAcSet,], type = 'response',na.action = na.omit)
plot(Test_AcOnly.set$date[compAcSet],pred,ylim=c(0,.7))
lines(Test_AcOnly.set$date[compAcSet],runmed(Test_AcOnly.set$Density[compAcSet],11), type="b", col="red")

# Predict on train data at MC, for comparison...
compAcSet <- which(ceiling(Train_AcOnly.set$long)==-84)
pred <- predict.gam(gam_full_AcOnly_TF$gam,transformedCovars_AcOnly.train[compAcSet,], type='response',na.action = na.omit)
plot(Train_AcOnly.set$date[compAcSet],pred,ylim=c(0,.85))
lines(Train_AcOnly.set$date[compAcSet],runmed(Train_AcOnly.set$Density[compAcSet],21), type="b", col="red")
...
encounterAll_gamm_TF <- gamm(yTF ~ s(SST, bs="ts",k=kVal)+ s(SSH, bs="ts",k=kVal)+ s(log10_CHL, bs="ts",k=kVal)
                             ...
                             na.action = na.omit,family=binomial(),control = list(keepData=TRUE,maxIter=50))#,family= Tweedie(p=1.4)
