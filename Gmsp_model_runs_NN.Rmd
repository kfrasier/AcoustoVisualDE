---
title: 'Pilot whale habitat models'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
plot.covarDensity(transformedCovars.train[,1:(length(transformedCovars.train)-4)],
                  colnames(transformedCovars.train[,1:(length(transformedCovars.train)-4)]),
                  mergedTrain.set$Density,paste0(outDir,'Both_',SP))

plot.covarDensity(transformedCovars_AcOnly.train[,1:(length(transformedCovars_AcOnly.train)-4)],
...
plot.covarDensity(transformedCovars_VisOnly.train[,1:(length(transformedCovars_VisOnly.train)-4)],
...
* Acoustic data have a trucation distance  of 1.9km for pilot whales (95% of detections occur within this range), for a total monitoring area of $(1.9 km)^{2} \pi = 11.3 km^{2}$, 
...
* Visual trucation distance for pilot whales is estimated as 6.5 km (see figure below) from these data. ESW is multiplied by transect segment length (usually ~10km) and doubled to estimate total area swept.


<br>  

\[\text{Acoustic to Visual ratio} = \frac{24 hrs \cdot (1.9 km)^{2} \pi}{(\text{Segment length}/18.52 km/hr) \cdot 6.5 km \cdot \text{Segment length} \cdot 2} = 272.18/70.2 \approx 3.9:1
\]
...
weeklyOccurrenceFile = paste0(outDir,'ALLSITES_weeklyPOccurrence_Gmsp_jahStart.csv')
...
```{r other models, eval = FALSE, echo = FALSE} 
## VISUAL ONLY
modelCounter <- 0
# put together the formula
...
predVisOnly_DC <- predict(nn_VisOnly[[best_VisOnly_ModelIndex]],AcOnly_test_scaled[compAcSet_DC,model1.indices])
predVisOnly_MP <- predict(nn_VisOnly[[best_VisOnly_ModelIndex]],AcOnly_test_scaled[compAcSet_MP,model1.indices])
...
july2009_AcOnly_prediction <- raster::predict(july2009_rasters,nn_AcOnly[[best_AcOnly_ModelIndex]],
                                na.action = na.pass)    
```
...
jan2009_VisOnly_prediction <- raster::predict(jan2009_rasters,nn_VisOnly[[best_VisOnly_ModelIndex]],
                                 na.action = na.pass)
