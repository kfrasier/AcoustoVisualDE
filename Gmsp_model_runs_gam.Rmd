---
title: 'Pilot whale habitat models: GAMs'
output: 
  html_document:
    toc: true
    toc_depth: 4
...
![](E:/NASData/ModelData/Gmsp/AcousticOnly_Gmsp_missingData.png)
<br> 
...
plot.cleveland(Train_Joint.set,covarList,FALSE,paste0(outDir,'AcousticAndVisual_',SP))
...
The acoustic data represent presence or absence of Cuvier's beaked whale detections in one day bins. Given that a group of Cuvier's is present near the sensor, the probability of detecting them in a 5 minute period is estimated at 0.65, therefore the probability of missing is 1-0.65 = 0.35. Given that animals were present, the probability of missing a group for a full day is estimated as
...
Models were fit using gam (single data type models) and gamm )joint models from the mgcv package in R. 
...
for (iFac in 1:length(transformedCovars_VisOnly.train$fac1)) {
  if (transformedCovars_VisOnly.train$Density[iFac]==0){
    # if it's visual data and it's a zero, adjust by g0 ie, only a X% chance it was a true zero.
...
for (iFac in 1:length(transformedCovars_VisOnly.test$fac1)) {
  if (transformedCovars_VisOnly.test$Density[iFac]==0){
    # if it's visual data and it's a zero, adjust by g0 ie, only a X% chance it was a true zero.
...
gam_full_AcOnly_TF[[1]] <- gam(yAcOnly_TF ~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
...
gam_full_VisOnly_TF[[1]] <- gam(yVisOnly_TF~ s(SST, bs='ts', k=kVal)
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
...
gam_full_Joint_TF[[1]] <- gam(y_TF~ s(SST, bs='ts', k=kVal) 
                               + s(SSH, bs='ts', k=kVal)
                               + s(log10_CHL, bs='ts', k=kVal)
                               + s(log10_HYCOM_MLD, bs='ts', k=kVal)
                               + s(HYCOM_SALIN_0,bs='ts', k=kVal)
...
cat('Best acoustic only model is #', best_AcOnly_model_index,' \n')
```
<br>
...
cat('Best visual only model is #', best_VisOnly_model_index,' \n')
```
...
for (iMod in 1:length(gam_full_Joint_TF)){
  Joint_model_AIC[iMod] <- AIC(gam_full_Joint_TF[[iMod]])
...
best_Joint_model <- gam_full_Joint_TF[[best_Joint_model_index]]
best_Joint_model_name <- names(gam_full_Joint_TF[best_Joint_model_index])
...
AcOnly_predictionSet$DC[m3] <-predAcOnly_MP$fit
...
p2_Ac <- ggplot() +
  ...
  scale_color_manual("",values= c("gray48","#009999"))+
  labs(y = "Encounter probabilit", x = "")+theme_bw()+
...
p4_Vis <- ggplot() +
  ...
  geom_line(data = pOccur[occurIdx,],
             aes(x =as.POSIXct(dateStr), y = DC, color = Legend))+
  geom_line(data = VisOnly_predictionSet,
            aes(x=date, y=DC, color = Legend)) +  scale_color_manual(values= c("gray48","#009999"))+
  labs(y = "Encounter probability", x = "")+theme_bw()+
  theme(legend.position="none")

p5_Vis <- ggplot() +
  geom_ribbon(data = VisOnly_predictionSet, 
            aes(x = date, ymax = predSd_MP + MP,ymin = pmax(DT - predSd_MP,0)), fill = "lightgray") +
  geom_area(data = VisOnly_predictionSet, 
            aes(x = date, y = pmax(MP - predSd_MP,0)), fill = "white") +
  geom_line(data = pOccur[occurIdx,],
             aes(x =as.POSIXct(dateStr), y = MP, color = Legend))+
...
p5 <- ggplot() +
  ...
  geom_line(data = Joint_predictionSet,
            aes(x=date, y = MP, color = Legend)) +
  scale_color_manual(values= c("gray48","#009999"))+theme_bw()+
  labs(y = "Encounter probability", x = "Date")+
  theme(legend.position="none")
