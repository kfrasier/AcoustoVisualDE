...
rownames(RMSEtable)<- c("Acoustic - Joint test data","Visual - Joint test data","Joint - Test")#"Acoustic - Train","Acoustic - Test",
...
best_AcOnly_ModelIndex <-which.min(RMSE$nn_AcOnly_test_allData)
print(paste(c('Best acoustic model has', layerSizeList[best_AcOnly_ModelIndex], 
              'nodes.'),collapse = " "))
best_VisOnly_ModelIndex <-which.min(RMSE$nn_VisOnly_test_allData)
print(paste(c('Best visual model has', layerSizeList[best_VisOnly_ModelIndex],
            'nodes.'),collapse = " "))
best_Joint_ModelIndex <-which.min(RMSE$nn_Joint_test_allData)
...
plot.nnet(nn_Joint[[best_Joint_ModelIndex]]$model[[bestNetwork]],x.lab = n3, y.lab = "P(E)",bias = F,cex=1.4,circle.col='lightskyblue1', node.labs = FALSE,circle.cex = 3, pad_x = 1,alpha.val =.8)
dev.off()
